...
$MapeamentoDocs = @{
    ...
    # Areas (Razor Pages)
    "Areas/.*/Pages/.*\.cshtml$"     = "Documentacao/Areas/"
    "Areas/.*/Pages/.*\.cshtml\.cs$" = "Documentacao/Areas/"
...
function Get-DocumentacaoPath {
    ...
    foreach ($pattern in $MapeamentoDocs.Keys) {
        if ($FilePath -match $pattern) {
            ...
            # Para Pages: tentar primeiro com estrutura de subdiretorio
            if ($FilePath -match "Pages/([^/]+)/(.+)\.(cshtml|cs)$") {
                $moduleName = $matches[1]
                $pageName = [System.IO.Path]::GetFileNameWithoutExtension($matches[2])

                # Se for PageModel (.cshtml.cs), preservar sufixo .cshtml
                if ($FilePath -match "\.cshtml\.cs$") {
                    $pageName = "$pageName.cshtml"
                }

                # Tentar formatos:
                # 1. Documentacao/Pages/ModuleName - PageName.md
                $docPath1 = Join-Path $basePath "$moduleName - $pageName.md"
                if (Test-Path $docPath1) { return $docPath1 }

                # 2. Documentacao/Pages/ModuleName/PageName.md
                $docPath2 = Join-Path $basePath "$moduleName/$pageName.md"
                if (Test-Path $docPath2) { return $docPath2 }
            }

            # Para Areas/Pages: manter estrutura completa do arquivo (incluindo .cshtml.cs)
            if ($FilePath -match "Areas/([^/]+)/Pages/(.+)$") {
                $areaName = $matches[1]
                $pageRelative = $matches[2]

                # Documentacao/Areas/{Area}/Pages/{CaminhoCompleto}.md
                $docPathArea = Join-Path $basePath "$areaName/Pages/$pageRelative.md"
                if (Test-Path $docPathArea) { return $docPathArea }
            }

            # Fallback: formato antigo (arquivo sem subdiretorio)
            $docPath = Join-Path $basePath "$fileName.md"
            if (Test-Path $docPath) {
                return $docPath
            }
                # Fallback extra: nome completo com extensao (ex: MotoristaCache.cs.md)
                $docPathWithExtension = Join-Path $basePath "$([System.IO.Path]::GetFileName($FilePath)).md"
                if (Test-Path $docPathWithExtension) {
                    return $docPathWithExtension
                }
        }
    }
