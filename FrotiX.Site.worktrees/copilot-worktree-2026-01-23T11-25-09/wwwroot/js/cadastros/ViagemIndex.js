...
/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║ 📌 NOME: abrirModalFicha                                                     ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║ 📝 DESCRIÇÃO:                                                                ║
 * ║    Abre o modal de ficha de vistoria e carrega a ficha da viagem se existir ║
 * ║                                                                              ║
 * ║ 🎯 IMPORTÂNCIA PARA A SOLUÇÃO:                                              ║
 * ║    Permite visualização e upload de fichas de vistoria de viagens          ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║ 📥 PARÂMETROS:                                                               ║
 * ║    • viagemId (Guid): ID da viagem para carregar/salvar a ficha            ║
 * ║                                                                              ║
 * ║ 📤 RETORNO:                                                                  ║
 * ║    • void                                                                    ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║ 📞 FUNÇÕES QUE CHAMA:                                                        ║
 * ║    • resetModalFicha() → Limpa o modal antes de abrir                      ║
 * ║    • carregarFichaVistoria(viagemId) → Carrega ficha existente se houver  ║
 * ║                                                                              ║
 * ║ 📲 CHAMADA POR:                                                              ║
 * ║    • Botão "Ficha de Vistoria" no DataTable (onclick do btn-foto)          ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║ 🔗 ESCOPO: EXTERNA - relaciona com Index.cshtml e modalFicha               ║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
function abrirModalFicha(viagemId) {
    try {
        // [VALIDAÇÃO] Verifica se o ID da viagem foi fornecido
        if (!viagemId || viagemId === 'undefined') {
            Alerta.Erro('Erro', 'ID da viagem não informado');
            return;
        }

        // [DADOS] Armazena o ID da viagem no modal e na variável global
        ...
        // [UI] Reseta o modal para estado inicial
        resetModalFicha();

        // [AJAX] Carrega a ficha existente (se houver)
        carregarFichaVistoria(viagemId);

        // [UI] Abre o modal usando Bootstrap 5
    ...
    try {
        ...
        dataTableViagens = $('#tblViagem').DataTable({
            ...
            columns: [
                ...
                // COLUNA: Ações (ícones em botões 28x28, Bootstrap 5)
                {
                    data: 'viagemId',
                    orderable: false,
                    searchable: false,
                    render: function (data, type, row, meta) {
                        try {
                            ...
                            // [LOGICA] Verifica se viagem tem ficha de vistoria (trata null, undefined, false, 0)
                            const temFicha =
                            ...
                            // Tooltips dinâmicos baseados no estado
            ...
            drawCallback: function (settings) {
                try {
                    // [UI] Reinicializa os tooltips Syncfusion após cada redesenho da tabela
                    // IMPORTANTE: Funciona também em elementos desabilitados (disabled)
                    var tooltipElements =
                        document.querySelectorAll('[data-ejtip]');
                    tooltipElements.forEach(function (element) {
                        // [LOGICA] Verifica se tooltip já foi criado para este elemento
                        if (
                            !element.ej2_instances ||
                            element.ej2_instances.length === 0
                        ) {
                            // [UI] Cria tooltip Syncfusion com suporte a elementos desabilitados
                            new ej.popups.Tooltip({
                                content: element.getAttribute('data-ejtip'),
                                position: 'TopCenter',
                                opensOn: 'Hover', // Exibe no hover
                                // IMPORTANTE: Não usar target, pois isso afeta o elemento direto
                                // Tooltips devem funcionar mesmo em elementos com classe 'disabled'
