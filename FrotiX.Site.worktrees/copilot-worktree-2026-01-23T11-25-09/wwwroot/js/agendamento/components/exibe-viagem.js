...
function preencherDataFinalRecorrencia(dataFinalRecorrencia, { disable = false, contexto = "" } = {})
{
    try
    {
        if (!dataFinalRecorrencia)
        {
            console.warn(`⚠️ Data Final Recorrência vazia${contexto ? ` (${contexto})` : ""}`);
            return false;
        }

        const txtFinalRecorrencia = document.getElementById("txtFinalRecorrencia");
        console.log(`🔍 DEBUG Data Final Recorrência${contexto ? ` (${contexto})` : ""}:`, dataFinalRecorrencia);
        console.log("  - txtFinalRecorrencia existe?", !!txtFinalRecorrencia);

        if (!txtFinalRecorrencia)
        {
            console.error("❌ Campo txtFinalRecorrencia não encontrado no DOM!");
            return false;
        }
        ...
        {
            console.warn("⚠️ Data Final Recorrência inválida:", dataFinalRecorrencia);
            return false;
        }

        // Usar API Kendo/Telerik para DatePicker
        ...
        {
            const dataFormatada = formatarDataParaInput(dataFinal, txtFinalRecorrencia.type);
    ...
    catch (error)
    {
        ...
        Alerta.TratamentoErroComLinha("exibe-viagem.js", "preencherDataFinalRecorrencia", error);
        return false;
    }
}
...
window.ExibeViagem = function (objViagem, dataClicada = null, horaClicada = null)
{
    try
    {
        ...
        // PASSO 4: ✅ CRÍTICO - Remover qualquer overlay de loading pendente (20/01/2026)
        // Isso evita que o modal fique bloqueado se houve erro anterior
        if (typeof window.esconderLoadingRelatorio === 'function')
        {
            window.esconderLoadingRelatorio();
        }
        // Também remover diretamente caso a função não exista ainda
        $('#modal-relatorio-loading-overlay').remove();

        // PASSO 5: Esconder FtxSpin global se estiver ativo
        if (window.FtxSpin && typeof window.FtxSpin.hide === 'function')
        {
            window.FtxSpin.hide();
        }

        // PASSO 6: Remover qualquer overlay .ftx-spin-overlay órfão
        $('.ftx-spin-overlay').each(function() {
            // Não remover se for o FtxSpin principal (ele usa display:none, não remove)
            if (this.id !== 'ftx-spin-overlay') {
                $(this).remove();
            }
        });

        // PASSO 7: Remover spinners e overlays Syncfusion órfãos que podem bloquear interação
        document.querySelectorAll('.e-spinner-pane, .e-spin-overlay, .e-spin-show, .e-overlay').forEach(function(el) {
            // Remover apenas se estiver no body (órfão)
        ...
        // PASSO 8: Garantir que nenhum overlay Kendo está bloqueando
        $('.k-overlay, .k-loading-mask').remove();

        // PASSO 9: Remover overlays Syncfusion órfãos diretamente do body
 ...
 * VERSÃO ATUALIZADA 20/01/2026 - Usa AgendamentoV2 (Kendo)
 */
function inicializarLstRecorrente()
{
    try
    {
        // Delega para AgendamentoV2 se disponível
        if (typeof window.AgendamentoV2 !== 'undefined' && window.AgendamentoV2._criarDropdownRecorrente)
        {
            console.log("🔄 [inicializarLstRecorrente] Delegando para AgendamentoV2");
            window.AgendamentoV2._criarDropdownRecorrente();
            return true;
        }

        // Fallback: verifica se já existe instância Kendo
        ...
        // Se não existe, cria novo Kendo DropDownList
...
function exibirNovaViagem(dataClicada, horaClicada)
{
    try
    {
        ...
        setTimeout(() =>
        {
            ...
            // ============================================
            // 9.5 MOSTRAR CARD DE RECORRÊNCIA (NOVO AGENDAMENTO)
            // ============================================
            const cardRecorrencia = document.getElementById("cardRecorrencia");
            if (cardRecorrencia)
            {
                cardRecorrencia.style.display = "block";
                console.log("✅ Card de recorrência VISÍVEL (novo agendamento)");
            }

            // Garantir que lstRecorrente esteja habilitado e com valor padrão "Não"
            ...
            if (lstRecorrenteKendo)
            {
                ...
                console.log("✅ lstRecorrente habilitado e definido como 'Não'");
            }
            ...
            // Código não causa erro (verificação if), mas elementos não existem mais
            /*
            ...
            */
            ...
            if (txtDataInicial)
            {
                ...
                var kendoDataInicialPicker = $("#txtDataInicial").data("kendoDatePicker");
                if (kendoDataInicialPicker)
                {
                    kendoDataInicialPicker.value(dataParaUsar);
                    kendoDataInicialPicker.enable(true);
                    console.log("Data definida no Kendo DatePicker txtDataInicial");
                }
                else
                {
                    console.warn("Kendo DatePicker txtDataInicial nao encontrado");
                }

                /* BLOCO SYNCFUSION DESATIVADO
                if (false)
                {
                ...
                } else
                {
                    ...
                    console.log("Data definida na instancia existente");
                }
                BLOCO SYNCFUSION DESATIVADO */
            ...
            // ============================================
            // 14. HABILITAR RECORR�SNCIA (CONTROLADOR �NICO)
            // ============================================
            if (window.RecorrenciaController?.prepararNovo)
            {
                window.RecorrenciaController.prepararNovo();
                console.log("�o. RecorrǦncia preparada para novo agendamento (controller)");
            }
            else
            {
                console.warn("�s���? RecorrenciaController nǜo dispon��vel - recorrǦncia nǜo inicializada");
            }
...
function exibirViagemExistente(objViagem)
{
    try
    {
        ...
        // ============================================
        // CONTROLE DO CARD DE RECORRÊNCIA - 20/01/2026
        // - Edição NÃO recorrente: ESCONDER card inteiro
        // - Edição recorrente: MOSTRAR card (campos desabilitados)
        // ============================================
        ...
        if (cardRecorrencia)
        {
            ...
            {
                // É recorrente - mostrar card mas com campos desabilitados
                cardRecorrencia.style.display = "block";
                console.log("   ✅ Card de recorrência VISÍVEL (edição de agendamento recorrente)");

                // Desabilitar lstRecorrente em edição de recorrente
                const lstRecorrenteKendo = $("#lstRecorrente").data("kendoDropDownList");
                if (lstRecorrenteKendo)
                {
                    lstRecorrenteKendo.value('S');
                    lstRecorrenteKendo.enable(false);
                    console.log("   🔒 lstRecorrente desabilitado (edição recorrente)");
                }
            }
            else
            {
                // NÃO é recorrente - esconder card completamente
                cardRecorrencia.style.display = "none";
                console.log("   🚫 Card de recorrência ESCONDIDO (não é recorrente)");
            }
        }
        ...
        // 3. Preencher campos básicos (suporta camelCase e PascalCase)
        $("#txtViagemId").val(objViagem.viagemId ?? objViagem.ViagemId);
        $("#txtRecorrenciaViagemId").val(objViagem.recorrenciaViagemId ?? objViagem.RecorrenciaViagemId ?? "");
        $("#txtStatusAgendamento").val(objViagem.statusAgendamento ?? objViagem.StatusAgendamento);
        $("#txtUsuarioIdCriacao").val(objViagem.usuarioIdCriacao ?? objViagem.UsuarioIdCriacao ?? "");
        $("#txtDataCriacao").val(objViagem.dataCriacao ?? objViagem.DataCriacao ?? "");

        // 4. Datas e horas - Alterado em: 20/01/2026 - Suporte camelCase e PascalCase
        // Obter valores com fallback para ambas as convenções de nomenclatura
        ...
        {
            var kendoDataInicialPicker = $("#txtDataInicial").data("kendoDatePicker");
            console.log("   Kendo DatePicker encontrado:", !!kendoDataInicialPicker);
            ...
            if (kendoDataInicialPicker)
            {
                try
                {
                    ...
                    {
                        // CORRECAO 20/01/2026: Remover restricao de data minima para permitir edicao de datas passadas
                        // O min="@DateTime.Today" no CSHTML impede datas passadas, mas em edicao precisamos aceitar
                        kendoDataInicialPicker.min(new Date(1900, 0, 1)); // Permitir qualquer data
                        kendoDataInicialPicker.value(dataObj);
                        console.log("   Min removido para edicao, data preenchida:", dataObj.toLocaleDateString('pt-BR'));
                    }
                    else
                    {
                        console.warn("   Data inicial invalida, usando data atual");
            ...
            {
                console.warn("   ⚠️ Kendo DatePicker txtDataInicial NÃO encontrado!");
                // Tentar preencher via input nativo como fallback
                const inputNativo = document.querySelector('input[name="txtDataInicial"]');
                if (inputNativo)
                {
                    const dataObj = new Date(dataInicialValor);
                    if (!isNaN(dataObj.getTime()))
                    {
                        // Formato yyyy-MM-dd para input type="date" ou dd/MM/yyyy para text
                        const ano = dataObj.getFullYear();
                        const mes = String(dataObj.getMonth() + 1).padStart(2, '0');
                        const dia = String(dataObj.getDate()).padStart(2, '0');
                        inputNativo.value = `${dia}/${mes}/${ano}`;
                        console.log("   ✅ Data inicial preenchida via input nativo:", inputNativo.value);
                    }
                }
            }
        }
        ...
        {
            console.warn("   ⚠️ dataInicial é undefined/null no objeto de viagem");
        }
        ...
        if (dataFinalValor)
        {
            ...
            if (kendoDataFinalPicker)
            {
                try
                {
                    ...
                    {
                        console.warn("   ⚠️ Data final invalida, limpando campo");
        ...
        {
            console.log("   ℹ️ dataFinal é undefined/null (normal para agendamento)");
        }
        ...
        if (lstSetorRequisitanteAgendamento && lstSetorRequisitanteAgendamento.ej2_instances && lstSetorRequisitanteAgendamento.ej2_instances[0])
        {
            ...
            if (setorId)
            {
                ...
                // Setar valor após delay (aumentado para garantir que controle está pronto)
                setTimeout(() =>
                {
                    try
                    {
        ...
        if (requisitanteId)
        {
            // Telerik Kendo: usa $("#lstRequisitante").data("kendoComboBox")
        ...
        // 12.1 Botão Ficha de Vistoria - habilitar/desabilitar baseado em TemFichaVistoriaReal
        const btnFichaVistoria = document.getElementById("btnVisualizarFichaVistoria");
        ...
        if (btnFichaVistoria)
        {
            // Verifica se tem ficha real cadastrada (não a amarelinha padrão)
            ...
            if (temFichaReal)
            {
                ...
                btnFichaVistoria.setAttribute("data-ejtip", "Clique para visualizar/imprimir a Ficha de Vistoria desta viagem");
            }
            else
            {
                btnFichaVistoria.style.display = "none";
            ...
            // Armazena o ViagemId e NoFicha para uso no clique
                    ...
                    $.ajax({
                        ...
                        success: function (response)
                        {
                            ...
                            if (response.success && response.data)
                            {
                                ...
                                // Usar API Kendo/Telerik para DatePicker
                                var kendoDataInicioEventoPicker = $("#txtDataInicioEvento").data("kendoDatePicker");
                                console.log("txtDataInicioEvento elemento:", document.getElementById("txtDataInicioEvento"));
                                console.log("kendoDatePicker:", kendoDataInicioEventoPicker);

                                if (dataInicial && kendoDataInicioEventoPicker)
                                {
                                    try
                                    {
                                        ...
                                        if (!isNaN(dataObj.getTime()))
                                        {
                                            kendoDataInicioEventoPicker.value(dataObj);
                                            kendoDataInicioEventoPicker.enable(false);
                                            console.log("Data Inicio preenchida:", dataObj.toLocaleDateString('pt-BR'));
                                        }
                                        else
                                        {
                                            console.error("Data Inicio invalida:", dataInicial);
                                            kendoDataInicioEventoPicker.value(null);
                                        }
                                    } catch (error)
                                    {
                                        console.error("Erro ao preencher Data Inicio:", error);
                                        kendoDataInicioEventoPicker.value(null);
                                    }
                                }
                                else
                                {
                                    console.error("Nao foi possivel preencher Data Inicio - componente nao encontrado ou data vazia");
                                }
                                ...
                                // Usar API Kendo/Telerik para DatePicker
                                var kendoDataFimEventoPicker = $("#txtDataFimEvento").data("kendoDatePicker");
                                console.log("txtDataFimEvento elemento:", document.getElementById("txtDataFimEvento"));
                                ...
                                if (dataFinal && kendoDataFimEventoPicker)
                                {
                                    try
                                    {
                                        ...
                                        if (!isNaN(dataObj.getTime()))
                                        {
                                            kendoDataFimEventoPicker.value(dataObj);
                                            kendoDataFimEventoPicker.enable(false);
                                            console.log("Data Fim preenchida:", dataObj.toLocaleDateString('pt-BR'));
                                        }
                                        else
                                        {
                                            console.error("Data Fim invalida:", dataFinal);
                                ...
                                {
                                    console.error("Nao foi possivel preencher Data Fim - componente nao encontrado ou data vazia");
                                }
        ...
        // ====================================
        // 15. RESTAURAR CONTROLES DE RECORR�NCIA (CONTROLADOR �NICO)
        // ====================================
        ...
        {
            console.warn("?? RecorrenciaController n�o dispon�vel - recorr�ncia n�o sincronizada. Aplicando fallback local.");
        ...
        window.timeoutAbrirModal = setTimeout(() =>
        {
            ...
            // CRÍTICO: Resetar flags após 1 segundo para garantir que tudo foi carregado
            setTimeout(() => {
                window.carregandoViagemExistente = false;
                console.log("✅ Flag carregandoViagemExistente resetada após modal aberto");
            }, 1000);
        }, 550);
    } catch (error)
    {
...
function configurarCamposRecorrencia(objViagem)
{
    try
    {
        ...
        // CORRIGIDO 20/01/2026: Suportar PascalCase e camelCase
        const intervalo = objViagem.Intervalo || objViagem.intervalo;
        console.log("🔧 Configurando campos de recorrência para tipo:", intervalo);
...
function configurarRecorrenciaSemanal(objViagem)
{
    try
    {
        ...
        if (lstDiasKendo)
        {
            // ✅ CRÍTICO: Popular lstDias se ainda não estiver populado
            ...
            // ✅ CORRIGIDO: Usar PascalCase e camelCase para compatibilidade
            ...
            // Mapear para valores do dataSource (0-6 conforme recorrencia-init.js)
            if (monday) diasSelecionados.push(1); // Segunda = 1
            if (tuesday) diasSelecionados.push(2); // Terça = 2
            if (wednesday) diasSelecionados.push(3); // Quarta = 3
            if (thursday) diasSelecionados.push(4); // Quinta = 4
            if (friday) diasSelecionados.push(5); // Sexta = 5
            if (saturday) diasSelecionados.push(6); // Sábado = 6
            if (sunday) diasSelecionados.push(0); // Domingo = 0

            console.log("📋 Dias selecionados (valores):", diasSelecionados);

            // Aguardar um pouco para garantir que o dataSource foi populado
            setTimeout(() => {
                lstDiasKendo.value(diasSelecionados);
                lstDiasKendo.enable(false);
                console.log("✅ lstDias (Kendo) configurado com dias selecionados");
            }, 100);
        }
        else
        {
            console.warn("⚠️ lstDias (Kendo) não encontrado ou não inicializado");
        }
...
function mostrarCamposViagem(objViagem)
{
    try
    {
        // Mostrar campos iniciais (divNoFichaVistoria foi removido - agora usa botão ao lado do Destino)
        console.log("🔴 TESTE CLAUDE: divNoFichaVistoria NÃO deve aparecer! Versão 21/01/2026 15:30");
...
function configurarRecorrencia(objViagem)
{
    try
    {
        ...
        // ✅ CORREÇÃO 20/01/2026: lstRecorrente agora é Kendo DropDownList
        const lstRecorrenteKendo = $("#lstRecorrente").data("kendoDropDownList");
        if (lstRecorrenteKendo)
        {
            lstRecorrenteKendo.value("S");
...
function mostrarCamposRecorrenciaSemanal(objViagem)
{
    try
    {
        ...
        if (lstDiasKendo)
        {
            ...
            // Mapear os dias booleanos para valores numéricos (0-6)
...
function restaurarDadosRecorrencia(objViagem)
{
    try
    {
        ...
        const intervaloValor = objViagem.Intervalo ?? objViagem.intervalo ?? objViagem.periodo ?? objViagem.Periodo;
        const dadosRecorrencia = {
            ...objViagem,
            Recorrente: objViagem.Recorrente ?? objViagem.recorrente ?? objViagem.ehRecorrente ?? objViagem.EhRecorrente,
            Intervalo: intervaloValor ? intervaloValor.toString().trim().toUpperCase() : intervaloValor,
            DataFinalRecorrencia: objViagem.DataFinalRecorrencia ?? objViagem.dataFinalRecorrencia,
            DiaMesRecorrencia: objViagem.DiaMesRecorrencia ?? objViagem.diaMesRecorrencia ?? objViagem.diaDoMes,
            diaDoMes: objViagem.diaDoMes ?? objViagem.DiaMesRecorrencia ?? objViagem.diaMesRecorrencia,
        ...
        const recorrenteTexto = (recorrenteValor ?? "").toString().trim().toUpperCase();
        const ehRecorrente = recorrenteValor === true ||
            recorrenteValor === 1 ||
            recorrenteValor === "1" ||
            recorrenteTexto === "S" ||
            recorrenteTexto === "SIM" ||
        ...
        if (ehRecorrente)
        {
            ...
            {
                lstRecorrenteKendo.value('S');
                lstRecorrenteKendo.enable(false); // Desabilitar em edição
            ...
            if (objViagem.Intervalo)
            {
                ...
                {
                    lstPeriodosKendo.value(objViagem.Intervalo);
                    lstPeriodosKendo.enable(false); // Desabilitar em edição de recorrência
                    console.log("✅ Período = " + objViagem.Intervalo + " (Kendo)");
                }
                ...
                if (objViagem.Intervalo === 'S' || objViagem.Intervalo === 'Q')
                {
                    ...
                    setTimeout(() =>
                    {
                        ...
                        if (lstDiasKendo)
                        {
                            ...
                            // Usar valores numéricos (0-6) para Kendo
                            if (objViagem.Sunday || objViagem.sunday) diasSelecionados.push(0);
                            if (objViagem.Monday || objViagem.monday) diasSelecionados.push(1);
                            if (objViagem.Tuesday || objViagem.tuesday) diasSelecionados.push(2);
                            if (objViagem.Wednesday || objViagem.wednesday) diasSelecionados.push(3);
                            if (objViagem.Thursday || objViagem.thursday) diasSelecionados.push(4);
                            if (objViagem.Friday || objViagem.friday) diasSelecionados.push(5);
                            if (objViagem.Saturday || objViagem.saturday) diasSelecionados.push(6);

                            if (diasSelecionados.length > 0)
                            {
                                lstDiasKendo.value(diasSelecionados);
                                lstDiasKendo.enable(false);
                                console.log("✅ [FINAL] lstDias (Kendo) preenchido:", diasSelecionados);
                            }
                        }

                        // ========================================
                        // ✅ CORREÇÃO 20/01/2026: Kendo renderiza chips automaticamente
                        // ========================================
                        setTimeout(() =>
                        {
                            console.log("📅 [RENDER] Verificando renderização Kendo...");
                            ...
                            {
                                lstPeriodosKendo.enable(false);
                                console.log("✅ [RENDER] lstPeriodos desabilitado (Kendo)");
                            }
                ...
                if (objViagem.Intervalo === 'M')
                {
                    ...
                    console.log("📅 Preenchendo lstDiasMes com dados de recorrência...");
                    ...
                    if (objViagem.diaDoMes)
                    {
                        ...
                        if (lstDiasMesKendo)
                        {
                            ...
                            lstDiasMesKendo.enable(false);
                            console.log("✅ lstDiasMes = " + objViagem.diaDoMes + " (Kendo)");
                        }
                    }

                    // Preencher data final no input date HTML
                    preencherDataFinalRecorrencia(objViagem.DataFinalRecorrencia, {
                        disable: true,
...
async function restaurarRecorrenciaVariada(objViagem)
{
    try
    {
        const recorrenciaId = objViagem?.RecorrenciaViagemId ?? objViagem?.recorrenciaViagemId;
        ...
        const calDatasSelecionadasEl = document.getElementById("calDatasSelecionadas");
        if (calDatasSelecionadasEl)
        {
            calDatasSelecionadasEl.style.setProperty('display', 'block', 'important');
        }

        const labelCalendario = document.getElementById("lblSelecioneAsDatas") ||
...
async function configurarRodapeLabelsExistente(objViagem)
{
    try
    {
        ...
        // DEBUG DETALHADO - 20/01/2026
        console.log("🔍 [configurarRodapeLabelsExistente] ==========================================");
        console.log("🔍 [configurarRodapeLabelsExistente] DADOS RECEBIDOS DO OBJETO:");
        console.log("   - status (camel):", objViagem.status, "| Status (Pascal):", objViagem.Status);
        console.log("   - statusAgendamento (camel):", objViagem.statusAgendamento, "| StatusAgendamento (Pascal):", objViagem.StatusAgendamento);
        console.log("   - foiAgendamento (camel):", objViagem.foiAgendamento, "| FoiAgendamento (Pascal):", objViagem.FoiAgendamento);
        console.log("   - usuarioIdAgendamento (camel):", objViagem.usuarioIdAgendamento, "| UsuarioIdAgendamento (Pascal):", objViagem.UsuarioIdAgendamento);
        console.log("   - dataAgendamento (camel):", objViagem.dataAgendamento, "| DataAgendamento (Pascal):", objViagem.DataAgendamento);
        console.log("   - usuarioIdCriacao (camel):", objViagem.usuarioIdCriacao, "| UsuarioIdCriacao (Pascal):", objViagem.UsuarioIdCriacao);
        console.log("   - dataCriacao (camel):", objViagem.dataCriacao, "| DataCriacao (Pascal):", objViagem.DataCriacao);
        console.log("🔍 [configurarRodapeLabelsExistente] ==========================================");

        // REGRA 1: Agendado Por - quando É um agendamento OU FOI um agendamento
        // CORREÇÃO 20/01/2026: Incluir statusAgendamento na verificação
        // - FoiAgendamento = true → viagem que FOI agendamento (já realizada)
        // - StatusAgendamento = true → AINDA É um agendamento ativo
        // Em ambos os casos, exibir "Agendado por" com usuarioIdAgendamento e dataAgendamento
        const foiAgendamentoRaw = objViagem.foiAgendamento ?? objViagem.FoiAgendamento;
        const foiAgendamento = foiAgendamentoRaw === true ||
                               foiAgendamentoRaw === 1 ||
                               foiAgendamentoRaw === "1" ||
        ...
        const statusAgendamentoRaw = objViagem.statusAgendamento ?? objViagem.StatusAgendamento;
        const ehAgendamento = statusAgendamentoRaw === true ||
                              statusAgendamentoRaw === 1 ||
                              statusAgendamentoRaw === "1" ||
                              statusAgendamentoRaw === "true" ||
                              statusAgendamentoRaw === "True";

        // Mostrar "Agendado por" se É agendamento OU foi agendamento
        ...
        console.log("🔍 [configurarRodapeLabelsExistente] statusAgendamentoRaw:", statusAgendamentoRaw, "tipo:", typeof statusAgendamentoRaw);
        console.log("🔍 [configurarRodapeLabelsExistente] mostrarAgendadoPor (calculado):", mostrarAgendadoPor);
        ...
        if (mostrarAgendadoPor)
        {
            ...
            if (labelAgendamento)
            {
                // ✅ CORREÇÃO: Recuperar nome do usuário via AJAX (suporta camelCase e PascalCase)
                const usuarioIdAgendamento = objViagem.usuarioIdAgendamento ?? objViagem.UsuarioIdAgendamento;
                const dataAgendamento = objViagem.dataAgendamento ?? objViagem.DataAgendamento;
                const usuario = await recuperarNomeUsuario(usuarioIdAgendamento);

                // Se usuário for vazio/nulo, mostrar "Usuário não encontrado"
                let texto;
                if (usuario && usuario.trim() !== '')
                {
                    texto = formatarDataHora(dataAgendamento, usuario, 'Agendado');
                }
                else
                {
        ...
        // REGRA 2: Criado Por - quando NÃO é agendamento E NÃO foi agendamento
        // CORREÇÃO 20/01/2026: Usar mostrarAgendadoPor ao invés de foiAgendamento
        if (!mostrarAgendadoPor)
        {
            ...
            {
                // ✅ CORREÇÃO: Recuperar nome do usuário via AJAX (suporta camelCase e PascalCase)
                const usuarioIdCriacao = objViagem.usuarioIdCriacao ?? objViagem.UsuarioIdCriacao;
                const dataCriacao = objViagem.dataCriacao ?? objViagem.DataCriacao;
        ...
        if (statusViagem === "Realizada")
        {
            ...
            {
                // ✅ CORREÇÃO: Recuperar nome do usuário via AJAX (suporta camelCase e PascalCase)
                const usuarioIdFinalizacao = objViagem.usuarioIdFinalizacao ?? objViagem.UsuarioIdFinalizacao;
                const dataFinalizacao = objViagem.dataFinalizacao ?? objViagem.DataFinalizacao;
                const usuario = await recuperarNomeUsuario(usuarioIdFinalizacao);

                // Se usuário for vazio/nulo, mostrar "Usuário não encontrado"
                let texto;
                if (usuario && usuario.trim() !== '')
                {
                    texto = formatarDataHora(dataFinalizacao, usuario, 'Finalizado');
                }
                else
                {
        ...
        if (statusViagem === "Cancelada")
        {
            ...
            {
                // ✅ CORREÇÃO: Recuperar nome do usuário via AJAX (suporta camelCase e PascalCase)
                const usuarioIdCancelamento = objViagem.usuarioIdCancelamento ?? objViagem.UsuarioIdCancelamento;
                const dataCancelamento = objViagem.dataCancelamento ?? objViagem.DataCancelamento;
                const usuario = await recuperarNomeUsuario(usuarioIdCancelamento);

                // Se usuário for vazio/nulo, mostrar "Usuário não encontrado"
                let texto;
                if (usuario && usuario.trim() !== '')
                {
                    texto = formatarDataHora(dataCancelamento, usuario, 'Cancelado');
                }
                else
                {
...
{
    // Listener para quando o modal e mostrado (registrado uma vez)
    if (!window._lstRecorrenteListenerRegistrado) {
        $('#modalViagens').on('shown.bs.modal', function ()
        {
            // NAO reinicializar se estiver carregando viagem existente
...
// Garantir limpeza ao fechar modal (registrado uma vez)
$(document).ready(function ()
{
    ...
    // Ocultar label ao abrir modal (registrado uma vez)
...
// ====================================================================
// MODAL FICHA DE VISTORIA - Visualização e Impressão
// ====================================================================

/**
 * Abre o modal de visualização da Ficha de Vistoria
 * @param {string} viagemId - ID da viagem
 * @param {string|number} noFicha - Número da ficha (opcional, para exibição)
 */
window.abrirModalFichaVistoria = async function (viagemId, noFicha)
{
    try
    {
        ...
        // Exibir número da ficha no título
        if (spanNoFicha)
        {
        ...
        // Carregar imagem da ficha via API
        console.log("[FichaVistoria] Carregando imagem...");
        const response = await fetch(`/api/Viagem/ObterFichaVistoria?viagemId=${viagemId}`);
    ...
    catch (error)
    {
        ...
        Alerta.TratamentoErroComLinha("exibe-viagem.js", "abrirModalFichaVistoria", error);
    }
};

/* Função imprimirFichaVistoria removida em 21/01/2026
 * O modal agora exibe apenas a imagem com botão Fechar
 * Caso seja necessário no futuro, reimplementar a partir do backup
 */

// Event listeners para o modal de Ficha de Vistoria
$(document).ready(function ()
{
    ...
    // Clique no botão de visualizar ficha
    ...
    $("#modalFichaVistoria").on("hidden.bs.modal", function ()
    {
        ...
        if (semImagem)
        {
            ...
            // Restaurar conteúdo original caso tenha sido alterado por erro
            semImagem.innerHTML = `
