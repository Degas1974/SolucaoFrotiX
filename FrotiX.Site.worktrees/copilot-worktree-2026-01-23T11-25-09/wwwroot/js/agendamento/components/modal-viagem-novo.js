...
window.criarAgendamentoNovo = function ()
{
    try
    {
        ...
        // Obter instâncias dos componentes Kendo/Telerik para DatePickers
        ...
        // 20/01/2026 - REFATORAÇÃO: Controles de recorrência agora são Kendo
        const lstRecorrente = $("#lstRecorrente").data("kendoDropDownList");
        const lstPeriodos = $("#lstPeriodos").data("kendoDropDownList");
        const txtFinalRecorrencia = $("#txtFinalRecorrencia").data("kendoDatePicker");
        const lstDias = $("#lstDias").data("kendoMultiSelect");
        const calDatasSelecionadas = document.getElementById("calDatasSelecionadas")?.ej2_instances?.[0]; // Mantém Syncfusion
        const lstDiasMes = $("#lstDiasMes").data("kendoDropDownList");

        // Extrair valores (API Kendo usa .value() como metodo)
        ...
        // Processar recorrência (20/01/2026 - usando API Kendo .value())
        const recorrente = lstRecorrente ? lstRecorrente.value() : "N";
        const intervalo = lstPeriodos ? lstPeriodos.value() : "";

        // CORREÇÃO 20/01/2026: txtFinalRecorrencia é Kendo DatePicker, usar .value() como método
        let dataFinalRecorrencia = null;
        const dataFinalRecValue = txtFinalRecorrencia ? txtFinalRecorrencia.value() : null;
        ...
        // 20/01/2026 - lstDias agora é Kendo MultiSelect, usa .value()
        const lstDiasValue = lstDias ? lstDias.value() : [];
        if (lstDiasValue && Array.isArray(lstDiasValue) && lstDiasValue.length > 0)
        {
        ...
        // Processar dia do mês (para recorrência mensal) - 20/01/2026 Kendo API
...
window.criarAgendamentoEdicao = function (agendamentoOriginal)
{
    try
    {
        ...
        // Ler campos de recorrência do formulário (API Kendo)
        const txtFinalRecorrenciaInst = $("#txtFinalRecorrencia").data("kendoDatePicker");
        const dataFinalRecorrenciaValue = txtFinalRecorrenciaInst ? txtFinalRecorrenciaInst.value() : null;
...
window.criarAgendamentoViagem = function (agendamentoUnicoAlterado)
{
    try
    {
        ...
        // Data final (opcional) - API Kendo
        let dataFinal = "";
        var kendoDataFinalPicker = $("#txtDataFinal").data("kendoDatePicker");
        var dataFinalKendoValue = kendoDataFinalPicker ? kendoDataFinalPicker.value() : null;
        ...
        // Ler Data Final Recorrência do formulário (API Kendo)
        const txtFinalRecorrenciaInst2 = $("#txtFinalRecorrencia").data("kendoDatePicker");
        const dataFinalRecorrenciaValue2 = txtFinalRecorrenciaInst2 ? txtFinalRecorrenciaInst2.value() : null;
...
window.enviarAgendamento = async function (agendamento)
{
    try
    {
        ...
        if (agendamento.DataFinal)
        {
            ...
            {
                // Limpar campo Data Final no modal (API Kendo)
                var kendoDataFinalClear = $("#txtDataFinal").data("kendoDatePicker");
    ...
    try
    {
        try
        {
            ...
            if (objViagem)
            {
                ...
                // CORRECAO 20/01/2026: Removida manipulacao manual de backdrop/modal-open
                // Bootstrap gerencia isso automaticamente ao chamar modal("hide")
                // A manipulacao manual causava conflito e travamento do modal
...
function detectarAlteracaoDataInicial(agendamentoOriginal)
{
    try
    {
        ...
        // Obter data atual do formulário (API Kendo)
        const txtDataInicialKendoCheck = $("#txtDataInicial").data("kendoDatePicker");
        const txtDataInicialKendoValue = txtDataInicialKendoCheck ? txtDataInicialKendoCheck.value() : null;
        ...
        try
        {
            ...
            if (await window.ValidaCampos(agendamentoFinal.ViagemId))
            {
                ...
                if (resultado.success)
                {
                    ...
                    // Fechar modal - Bootstrap gerencia backdrop automaticamente
                    $("#modalViagens").modal("hide");
                } else
                {
...
window.editarAgendamentoRecorrente = async function (viagemId, editaTodos, dataInicialRecorrencia, recorrenciaViagemId, editarAgendamentoRecorrente)
{
    try
    {
        ...
        const fecharModalComSucesso = () =>
        {
            try
            {
                // CORRECAO 20/01/2026: Bootstrap gerencia backdrop automaticamente
                $("#modalViagens").modal("hide");
                ...
                if (window._recorrenciaOverlayAtivo)
                {
                    setTimeout(() =>
                    {
                        ...
                        {
                            Alerta.TratamentoErroComLinha("modal-viagem.js", "fecharModalComSucesso_hideOverlay", error);
                        }
                    }, 200);
                }
                ...
                if (mensagemToastRecorrencia)
                {
                    setTimeout(() =>
                    {
                        try
                        {
                            AppToast.show(corToastRecorrencia, mensagemToastRecorrencia, 4000);
                        } catch (error)
                        {
                            Alerta.TratamentoErroComLinha("modal-viagem.js", "fecharModalComSucesso_toast", error);
                        }
                    }, 300);
                }
            } catch (error)
        ...
        try
        {
            ...
            if (editaTodos)
            {
                ...
                if (recorrenciaViagemId === "00000000-0000-0000-0000-000000000000" || !recorrenciaViagemId)
                {
                    ...
                    const chavePrimeiro = primeiroDaSerie?.viagemId || primeiroDaSerie?.ViagemId || primeiroDaSerie?.id || primeiroDaSerie?.Id || `sem-id-${registrosProcessados.size + 1}`;
                ...
                for (const agendamentoRecorrente of agendamentos)
                {
                    ...
                    {
                        const chaveAtual = agendamentoRecorrente?.viagemId || agendamentoRecorrente?.ViagemId || agendamentoRecorrente?.id || agendamentoRecorrente?.Id || `sem-id-${registrosProcessados.size + 1}`;
            ...
            if (editaTodos)
            {
                if (totalTentativas > 0)
                {
                    ...
                    {
                        mensagemToastRecorrencia = "Nenhum agendamento recorrente foi atualizado.";
            ...
            } else if (window._recorrenciaOverlayAtivo)
            {
                ...
                {
                    Alerta.TratamentoErroComLinha("modal-viagem.js", "editarAgendamentoRecorrente_hideOverlay", error);
                }
            }
        } catch (error)
        {
            if (window._recorrenciaOverlayAtivo)
            {
                ...
                {
                    Alerta.TratamentoErroComLinha("modal-viagem.js", "editarAgendamentoRecorrente_hideOverlay", innerError);
                }
            }
...
/**
 * ✅ Exibe mensagem de sucesso e fecha modal
 * Usado após criação bem-sucedida de agendamentos
 * @param {number} quantidade - Número de agendamentos criados (padrão: 1)
 */
...
$(function ()
{
    ...
    // VALIDAÇÃO: Data Final não pode ser superior à data atual
    // Configura evento change para o DatePicker Kendo txtDataFinal
    const configurarValidacaoDataFinal = function ()
    {
        try
        {
            var kendoDataFinalPicker = $("#txtDataFinal").data("kendoDatePicker");
            if (kendoDataFinalPicker && !kendoDataFinalPicker._dataFinalValidacaoConfigurada)
            {
                // Adiciona evento change para validacao
                kendoDataFinalPicker.bind("change", function(e) {
                    try
                    {
                        ...
                        if (dataFinalValue)
                        {
                            ...
                            {
                                this.value(null);
                                AppToast.show("Amarelo", "A Data Final nao pode ser superior a data atual.", 4000);
                            }
                        }
                    }
                    catch (error)
                ...
                console.log("[ModalViagem] Validacao de Data Final configurada (change)");
            }
        }
        catch (error)
        {
    ...
    // CORRECAO 20/01/2026: Removido listener duplicado no document que causava travamento
    // O evento shown.bs.modal ja e tratado em aoAbrirModalViagem (linhas 2168-2173)
    // Manter apenas a configuracao imediata como fallback

    // Tenta configurar imediatamente (caso o componente ja exista)
...
window.inicializarCamposModal = function ()
{
    try
    {
        // Lista de IDs de controles especiais (Kendo/Syncfusion) que NÃO devem ter value alterado diretamente
        const controlesEspeciais = [
            ...
            'lstMotorista', 'lstVeiculo', 'lstRequisitante', 'lstSetorRequisitanteAgendamento',
            'lstFinalidade', 'lstEventos', 'rteDescricao',
            'calDatasSelecionadas', 'lstDiasCalendario'
        ];

        // Habilita todos os campos exceto o container de botões e controles especiais
        ...
        if (divModal)
        {
            ...
            {
                // Pula container de botões
                if (node.id === "divBotoes") continue;

                // Pula controles especiais (Kendo/Syncfusion)
                if (controlesEspeciais.includes(node.id)) continue;

                // Pula elementos que são parte dos controles Kendo (têm classes k-)
                if (node.className && typeof node.className === 'string' &&
                    (node.className.includes('k-') || node.className.includes('e-'))) continue;

                // Habilita e limpa apenas campos de input simples
                node.disabled = false;
...
window.limparCamposRecorrencia = function ()
{
    try
    {
        // ✅ CORREÇÃO 20/01/2026: Componentes de recorrência agora são Kendo (exceto calDatasSelecionadas que é Syncfusion)

        // Limpar lstRecorrente (Kendo DropDownList)
        const lstRecorrente = $("#lstRecorrente").data("kendoDropDownList");
        if (lstRecorrente) {
            lstRecorrente.value("N");
        }

        // Limpar lstPeriodos (Kendo DropDownList)
        const lstPeriodos = $("#lstPeriodos").data("kendoDropDownList");
        if (lstPeriodos) {
            lstPeriodos.value("");
        }

        // Limpar lstDias (Kendo MultiSelect)
        const lstDias = $("#lstDias").data("kendoMultiSelect");
        if (lstDias) {
            lstDias.value([]);
        }

        // Limpar lstDiasMes (Kendo DropDownList)
        const lstDiasMes = $("#lstDiasMes").data("kendoDropDownList");
        if (lstDiasMes) {
            lstDiasMes.value("");
        }

        // Limpar txtFinalRecorrencia (Kendo DatePicker)
        const txtFinalRecorrencia = $("#txtFinalRecorrencia").data("kendoDatePicker");
        if (txtFinalRecorrencia) {
            txtFinalRecorrencia.value(null);
            txtFinalRecorrencia.enable(true); // Garantir que está habilitado
        }

        // Limpar calDatasSelecionadas (Syncfusion Calendar - mantém API EJ2)
...
window.limparCamposModalViagens = function ()
{
    try
    {
        ...
        // Limpar campos de Equipamentos (Cabo e Arla)
        $("#hidCaboEntregue, #hidCaboDevolvido, #hidArlaEntregue, #hidArlaDevolvido").val("false");
        $("#chkCaboEntregue, #chkCaboDevolvido, #chkArlaEntregue, #chkArlaDevolvido").prop("checked", false);
        ...
        // ✅ CORREÇÃO 20/01/2026: lstDiasMes e lstDias agora são Kendo, NÃO Syncfusion
        const syncIds = ["lstFinalidade", "ddtSetor", "cmbOrigem", "cmbDestino", "lstMotorista", "lstVeiculo", "lstRequisitante", "lstSetorRequisitanteAgendamento", "lstEventos", "ddtCombustivelInicial", "ddtCombustivelFinal"];
        syncIds.forEach(id =>
        {
            try
            {
        ...
        // LIMPEZA DE CONTROLES KENDO (lstDiasMes, lstDias)
        // CORRECAO 20/01/2026: Estes controles agora sao Kendo, nao Syncfusion
        try {
        ...
        // Limpar datas - VERSÃO CORRIGIDA COM PROTEÇÃO PARA EDIÇÃO
        // NÃO limpar datas se estamos carregando viagem existente
        if (!window.carregandoViagemExistente)
        {
        ...
        {
            console.log("[limparCampos] Pulando limpeza de datas (carregando viagem existente)");
        }
        ...
        setTimeout(() =>
        {
            // CRÍTICO: Verificar flag DENTRO do timeout para evitar sobrescrever dados de edição
            if (window.carregandoViagemExistente)
            {
                console.log("📌 [limparCampos] Pulando reset lstRecorrente - edição em andamento");
                return;
            }
            ...
            try {
                ...
                if (lstRecorrenteKendo) {
                    ...
                    console.log("✅ [limparCampos] lstRecorrente (Kendo) definido como 'Não'");
                    window.ignorarEventosRecorrencia = false;
                } else {
        ...
        // Limpar período - VERSÃO CORRIGIDA
        ...
        // ✅ RESTAURAR input date HTML de Data Final Recorrência
        console.log("🔄 [ModalViagem] Restaurando input date de Data Final Recorrência...");
        const txtFinalRecorrencia = document.getElementById("txtFinalRecorrencia");

        if (txtFinalRecorrencia)
        {
            // Limpar valor e garantir que esteja visível e editável
...
window.desabilitarTodosControles = function ()
{
    try
    {
        ...
        // Desabilita componentes EJ2 (EXCETO os do modal-footer e os migrados para Kendo)
        // ✅ CORREÇÃO 20/01/2026: Removidos lstRecorrente, lstPeriodos, lstDias, lstDiasMes, txtDataInicial, txtFinalRecorrencia (agora são Kendo)
        const componentesEJ2 = [
            "txtDataFinal", "lstFinalidade",
        ...
        // ✅ CORREÇÃO 20/01/2026: Desabilitar controles Kendo separadamente
        try {
