...
// ====================================================================
// TOGGLE DEV - Data Final Recorrência
// ====================================================================
...
window.aplicarConfigRecorrencia = function () {
    try {
        ...
        if (typeof config.forcarDatePickerRecorrencia === 'boolean') {
            window.forcarDatePickerRecorrencia = config.forcarDatePickerRecorrencia;
        }

        if (config.forcarTextoRecorrencia === true && config.forcarDatePickerRecorrencia === true) {
            window.forcarDatePickerRecorrencia = false;
        }
    }
    catch (error) {
        Alerta.TratamentoErroComLinha("state.js", "aplicarConfigRecorrencia", error);
    }
};
...
window.inicializarToggleRecorrenciaDev = function () {
    try {
        ...
        const atualizarLabel = () => {
            ...
            lblToggle.textContent = textoAtual ? "Recorrência: Texto" : "Recorrência: DatePicker";
        };
        ...
        btnToggle.addEventListener("click", function () {
            try {
                ...
                if (typeof Alerta !== 'undefined' && Alerta.Warning) {
                    const modo = window.forcarTextoRecorrencia ? "Texto readonly" : "DatePicker";
                    Alerta.Warning("Recorrência (DEV)", `Modo alternado para ${modo}.`);
                }
            }
            catch (error) {
                Alerta.TratamentoErroComLinha("state.js", "btnToggleRecorrenciaDev.click", error);
            }
        });
    }
    catch (error) {
        Alerta.TratamentoErroComLinha("state.js", "inicializarToggleRecorrenciaDev", error);
    }
};
