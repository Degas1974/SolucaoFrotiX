...
## [22/01/2026 14:40] - FIX: Remoção de Filtros de Status para Incluir Registros Ativos e Inativos

**Descrição**: Removidos os filtros de `Status` nas listas de Veículos, Motoristas e Eventos para permitir que TODOS os registros (ativos e inativos) sejam exibidos nos dropdowns.

**Motivação**: Permitir que usuários visualizem e selecionem registros inativos em formulários, especialmente útil para:
- Consultar viagens antigas com veículos/motoristas inativos
- Editar registros históricos
- Relatórios completos sem limitação de status

**Arquivos Afetados**:
- `ListaVeiculos.VeiculosList()` (linha 414)
- `ListaMotorista.MotoristaList()` (linha 524)
- `ListaEvento.EventosList()` (linha 705)

**Alterações Realizadas**:

1. **ListaVeiculos** (linhas 396-431):
```csharp
// ❌ ANTES (filtrava apenas ativos)
where v.Status == true

// ✅ DEPOIS (retorna todos)
// NOTA: Removido filtro de Status para incluir veículos ativos E inativos
select new ListaVeiculos
```

2. **ListaMotorista** (linhas 504-531):
```csharp
// ❌ ANTES (filtrava apenas ativos)
...
3. **ListaEvento** (linhas 689-712):
```csharp
// ❌ ANTES (filtrava apenas status "1")
...
- Comentários inline das classes atualizados para refletir "Ativos e Inativos"
- Seção `QUERY DATABASE` de cada classe atualizada com "Filtro: NENHUM"

**Impacto**: Positivo - Melhora flexibilidade do sistema sem quebrar funcionalidades existentes
