...
> **Última Atualização**: 22/01/2026 15:00
> **Versão Atual**: 1.7
...
document
  ...
  ?.addEventListener("focusout", calcularDuracaoViagem);
...
## [22/01/2026 15:00] - Correções de UI/UX: Bordas, Posicionamento e Organização de Campos

**Descrição**: Conjunto de correções de interface e organização de componentes na página de Upsert de Viagens:

1. **Bordas nos DropDownTree**: Adicionadas bordas (`border: 1px solid #ced4da`) e border-radius aos controles Syncfusion DropDownTree (Finalidade, Combustível Inicial, Combustível Final, Setor Solicitante) que estavam sem bordas superior e inferior.

2. **Reorganização de Campos Cabo/Arla**: Movidos os campos "Cabo Entregue" e "Arla Entregue" do card "Documentos e Itens Devolvidos" para o card correto "Documentos e Itens Entregues", onde deveriam estar desde o início. Mantidos apenas "Cabo Devolvido" e "Arla Devolvido" no card de Devolvidos.

3. **Estrutura de Cards**: Corrigida a estrutura HTML fechando adequadamente o card "Documentos e Itens Devolvidos" antes da seção Mobile. Os botões "Criar Viagem" e "Cancelar Operação" agora ficam fora do card, como esperado.

4. **Botão Zoom da Ficha de Vistoria**:
   - Removido CSS duplicado que posicionava o círculo branco no canto direito
   - Círculo agora alinhado corretamente ao canto esquerdo inferior (left: 8px; bottom: 8px)
   - Ajustado ícone da lupa para usar apenas `fa-duotone fa-magnifying-glass-plus` sem estilo inline

**Arquivos Afetados**:

- `Pages/Viagens/Upsert.cshtml` (linhas 776-779, 1628-1650, 1903-1907, 1148-1179, 2103)

**Impacto**:

- Melhoria visual dos controles de dropdown
- Organização correta dos campos de itens entregues vs devolvidos
- Melhor estrutura semântica do HTML
- Consistência no posicionamento do botão de zoom
...
## [22/01/2026 16:50] - DOC: Ajustes de formatação e limpeza

**Descrição**: Ajustes de formatação Markdown (títulos e blocos de código) e remoção de seção duplicada com texto corrompido.
...
## [22/01/2026 14:00] - FIX: Ajuste de rubrica inicial na view

**Descrição**: Corrigida referência da rubrica inicial na view para usar a propriedade existente `Rubrica` do modelo `Viagem`.

**Mudanças**:

1. **View**: Ajuste da verificação de rubrica inicial para `Model.ViagemObj.Viagem.Rubrica`
2. **Benefício**: Elimina erro de compilação (CS1061) e mantém a lógica de identificação de viagem mobile

**Arquivos Afetados**:

- `Pages/Viagens/Upsert.cshtml` (linha 1264)

**Impacto**: A tela volta a compilar e a detecção de rubrica inicial funciona corretamente.
