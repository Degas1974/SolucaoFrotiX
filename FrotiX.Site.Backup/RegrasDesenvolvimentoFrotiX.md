...
### 1.1 Scripts de Banco de Dados - FrotiXBDScripts.sql (OBRIGATÓRIO)

**Contexto:** O projeto possui um arquivo consolidado de scripts SQL (`Scripts/FrotiXBDScripts.sql`) que contém TODAS as alterações de banco de dados. Este arquivo é a fonte única de verdade para modificações de estrutura.

**Regra:** SEMPRE que um novo script SQL for criado ou uma alteração de banco for necessária:

1. **VALIDAR** se o elemento já existe em `FrotiXBDScripts.sql`
2. **ACRESCENTAR** a nova alteração ao arquivo consolidado
3. **USAR DROP + CREATE** para objetos que podem ser recriados (Views, Triggers, Stored Procedures, Functions)
4. **NUNCA** usar DROP em TABELAS - usar apenas ALTER TABLE para modificações
5. **Jobs** ficam em arquivo separado: `Scripts/FrotiXDBJobs.sql` (só executa no servidor)

**Política de DROP:**

| Objeto | DROP Permitido? | Ação |
|--------|-----------------|------|
| Views | ✅ SIM | DROP + CREATE |
| Triggers | ✅ SIM | DROP + CREATE |
| Stored Procedures | ✅ SIM | DROP + CREATE |
| Functions | ✅ SIM | DROP + CREATE |
| Foreign Keys | ✅ SIM | DROP + ADD (se necessário recriar) |
| Indices | ✅ SIM | DROP + CREATE (se necessário recriar) |
| **TABELAS** | ❌ **NUNCA** | Usar ALTER TABLE |

**Estrutura do FrotiXBDScripts.sql:**

```
SECAO 1: SCHEMAS
SECAO 2: TABELAS (CREATE IF NOT EXISTS - nunca DROP)
SECAO 3: COLUNAS NOVAS (ALTER TABLE)
...
**Exemplo de adição correta (View):**

```sql
-- SECAO 6: VIEWS (DROP + CREATE)

-- DROP e CREATE MinhaNovaView
IF OBJECT_ID('dbo.MinhaNovaView', 'V') IS NOT NULL
    DROP VIEW dbo.MinhaNovaView;
GO

CREATE VIEW dbo.MinhaNovaView AS
SELECT ... FROM ...;
GO
PRINT '+ View dbo.MinhaNovaView criada';
GO
```

**Exemplo de adição correta (Coluna):**

```sql
-- SECAO 3: COLUNAS NOVAS (ALTER TABLE)

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(N'dbo.MinhaTabela') AND name = 'NovaColuna')
BEGIN
    ALTER TABLE dbo.MinhaTabela ADD NovaColuna VARCHAR(100) NULL;
    PRINT '+ Coluna MinhaTabela.NovaColuna adicionada';
END
GO
```

**Data de Adição:** 23/01/2026
...
| ------ | ---------- | -------------------------------------------------------------------------------- |
| 1.4    | 23/01/2026 | Adiciona regra de Scripts BD consolidados (FrotiXBDScripts.sql) na seção 1.1     |
...
## Regra: Escape de @ em Arquivos Razor (.cshtml)

**Contexto:** Em arquivos Razor (.cshtml), o caractere `@` é reservado para sintaxe Razor (expressões C#, diretivas, etc.). Quando usado em JavaScript dentro de arquivos .cshtml, causa erros de compilação.

**Regra:** SEMPRE escapar o caractere `@` com `@@` quando usado em comentários JSDoc ou qualquer contexto JavaScript dentro de arquivos Razor.

**Problema Comum:**
```cshtml
<!-- ❌ ERRADO - Causa erro de compilação CS0103 -->
<script>
    /**
...
- `@global` → `@@global`
- `@since` → `@@since`
- Qualquer outra tag JSDoc com `@`

**Alternativa:** Mover JavaScript com JSDoc para arquivos `.js` separados em `wwwroot/js/`, onde `@` não precisa de escape.
