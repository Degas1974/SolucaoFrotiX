/* ╔════════════════════════════════════════════════════════════════════════════════════════════════════╗
   ...
   ╠════════════════════════════════════════════════════════════════════════════════════════════════════╣
   ║ 🎯 OBJETIVO DO ARQUIVO:                                                                            ║
   ║    DbContext principal do Entity Framework Core para acesso aos dados do FrotiX.                   ║
   ║    Mapeia entidades para tabelas SQL Server e aplica configurações de precisão.                    ║
   ╠════════════════════════════════════════════════════════════════════════════════════════════════════╣
   ║ 📋 ÍNDICE DE FUNÇÕES (Entradas -> Saídas):                                                         ║
   ║ 1. [FrotiXDbContext]               : Construtor do DbContext.......... (options) -> void            ║
   ║ 2. [OnModelCreating]               : Configuração do modelo EF........ (ModelBuilder) -> void      ║
   ║ 3. [ApplyDecimalPrecisionDefaults] : Define precision 18,2 decimals... (ModelBuilder) -> void      ║
   ╠════════════════════════════════════════════════════════════════════════════════════════════════════╣
   ║ ⚠️ MANUTENÇÃO:                                                                                     ║
...
namespace FrotiX.Data
{
    public partial class FrotiXDbContext : DbContext
    {
        ...
        public DbSet<TipoServico> TipoServico
        {
            get; set;
        }

        public DbSet<Turno> Turno
        {
            get; set;
        }

        public DbSet<VAssociado> VAssociado
        {
            get; set;
        }

        public DbSet<EscalaDiaria> EscalaDiaria
        {
            get; set;
        }

        public DbSet<FolgaRecesso> FolgaRecesso
        {
            get; set;
        }

        public DbSet<Ferias> Ferias
        {
            get; set;
        }

        public DbSet<ObservacoesEscala> ObservacoesEscala
        {
            get; set;
        }

        public DbSet<CoberturaFolga> CoberturaFolga
        {
            get; set;
        }

        public DbSet<ViewMotoristasVez> ViewMotoristasVez
        {
            get; set;
        }

        public DbSet<ViewStatusMotoristas> ViewStatusMotoristas
        {
            get; set;
        }

        public DbSet<ViewEscalasCompletas> ViewEscalasCompletas
        {
            get; set;
        }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<ViewStatusMotoristas>().HasNoKey();
            modelBuilder.Entity<ViewMotoristasVez>().HasNoKey();
            modelBuilder.Entity<ViewEscalasCompletas>().HasNoKey();
            ...
            // ================================================================
            // CONFIGURAÇÕES DE PRECISÃO DECIMAL (do arquivo parcial)
            // ================================================================
        ...
        // ╭───────────────────────────────────────────────────────────────────────────────────────╮
        // │ ⚡ FUNCIONALIDADE: ApplyDecimalPrecisionDefaults                                      │
        // │───────────────────────────────────────────────────────────────────────────────────────│
        // │ 🎯 DESCRIÇÃO: Aplica precisão padrão (18,2) para todas propriedades decimal.         │
        // │    Evita inconsistências entre modelo C# e SQL Server.                                │
        // │───────────────────────────────────────────────────────────────────────────────────────│
        // │ 📥 INPUTS: • modelBuilder [ModelBuilder]: Builder do modelo EF                       │
        // │ 📤 OUTPUTS: • void - Aplica configuração ao modelo                                   │
        // ╰───────────────────────────────────────────────────────────────────────────────────────╯
        
        private static void ApplyDecimalPrecisionDefaults(ModelBuilder modelBuilder)
        {
            // [DB] Itera por todas as entidades do modelo
            foreach (var entityType in modelBuilder.Model.GetEntityTypes())
            {
                // [DB] Localiza todas as propriedades do tipo decimal
                var decimalProperties = entityType.ClrType.GetProperties()
                    .Where(p => p.PropertyType == typeof(decimal) || p.PropertyType == typeof(decimal?));

                // [DB] Aplica configuração (18,2) para cada propriedade decimal
