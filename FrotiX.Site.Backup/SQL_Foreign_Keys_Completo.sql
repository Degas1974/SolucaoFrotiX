-- ============================================================================
-- SQL PARA ADICIONAR FOREIGN KEYS FALTANTES - COMPLETO
-- ============================================================================
--
-- Este script adiciona TODAS as Foreign Keys faltantes identificadas na
-- análise do banco de dados FrotiX.
--
-- Data de Criação: 12/01/2026
-- Gerado por: Claude Code
-- Baseado em: Analises/Relatorio_FKs_Indices_Faltantes.md
--
-- IMPORTANTE:
...
-- - Algumas FKs podem falhar se houver dados órfãos (registros inexistentes)
...
PRINT '============================================================================'
PRINT 'FOREIGN KEYS FALTANTES - IMPLEMENTAÇÃO COMPLETA'
PRINT '============================================================================'
PRINT ''
...
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_WhatsAppMensagens_UsuarioId')
BEGIN
    ALTER TABLE dbo.WhatsAppMensagens WITH NOCHECK
        ADD CONSTRAINT FK_WhatsAppMensagens_UsuarioId
        FOREIGN KEY (UsuarioId)
        REFERENCES dbo.AspNetUsers (Id)
    PRINT '✅ FK_WhatsAppMensagens_UsuarioId criada'
END
ELSE PRINT '✓  FK_WhatsAppMensagens_UsuarioId já existe'
GO
...
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_WhatsAppWebhookLogs_InstanciaId')
BEGIN
    ALTER TABLE dbo.WhatsAppWebhookLogs WITH NOCHECK
        ADD CONSTRAINT FK_WhatsAppWebhookLogs_InstanciaId
        FOREIGN KEY (InstanciaId)
        REFERENCES dbo.WhatsAppInstancias (InstanciaId)
    PRINT '✅ FK_WhatsAppWebhookLogs_InstanciaId criada'
END
ELSE PRINT '✓  FK_WhatsAppWebhookLogs_InstanciaId já existe'
GO
...
IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE name = 'FK_ItemVeiculoAta_VeiculoId')
BEGIN
    ALTER TABLE dbo.ItemVeiculoAta WITH NOCHECK
        ADD CONSTRAINT FK_ItemVeiculoAta_VeiculoId
        FOREIGN KEY (VeiculoId)
        REFERENCES dbo.Veiculo (VeiculoId)
    PRINT '✅ FK_ItemVeiculoAta_VeiculoId criada'
END
ELSE PRINT '✓  FK_ItemVeiculoAta_VeiculoId já existe'
GO
...
WHERE fk.name IN (
    ...
    'FK_WhatsAppMensagens_UsuarioId',
    'FK_WhatsAppFilaMensagens_MensagemId',
    'FK_WhatsAppWebhookLogs_InstanciaId',
    'FK_ItemVeiculoAta_RepactuacaoAtaId',
    'FK_ItemVeiculoAta_VeiculoId',
...
PRINT '3. Execute o script SQL_Indices_Performance.sql para criar índices'
PRINT '4. Teste as funcionalidades do sistema para validar as FKs'
PRINT '5. Após validar em desenvolvimento, aplique em PRODUÇÃO com cautela'
PRINT ''
GO
