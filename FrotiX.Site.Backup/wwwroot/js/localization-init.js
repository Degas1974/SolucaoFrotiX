window.loadSyncfusionLocalization = async function ()
{
    ...
    {
        // Carregar dados CLDR primeiro
        ej.base.loadCldr(...cldrData);

        // Definir cultura pt-BR
        ej.base.setCulture(culture);

        // Garantir que símbolos de formatação estejam disponíveis ANTES de qualquer componente
        if (ej.base.Internationalization) {
            const intl = new ej.base.Internationalization();
            // Forçar cache dos formatos para evitar erro ao formatar
            try {
                intl.getNumberFormat({ format: 'N0' }, culture);
                intl.getNumberFormat({ format: 'P0' }, culture);
                intl.getNumberFormat({ format: 'C0' }, culture);
            } catch (e) {
                console.warn('[Localization] Aviso ao pré-carregar formatos:', e);
            }
        }

        // Carregar strings localizadas
        ej.base.L10n.load({
            'pt-BR': {
                calendar: {
                    today: 'Hoje',
                ...
                numerictextbox: {
                    ...
                    // Formatos de número críticos para evitar erro "percentSign"
                    percentSymbol: '%',
                    currencySymbol: 'R$',
                    decimal: ',',
                    group: '.'
