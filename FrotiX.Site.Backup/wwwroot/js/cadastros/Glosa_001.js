...
$(document).ready(function () {
    try
    {
        ...
        // ====================================================================
        // EVENT HANDLER: SELEÇÃO DE CONTRATO
        // Quando o usuário seleciona um contrato, carrega os detalhes e exibe
        // as abas apropriadas conforme o tipo (Terceirização, Locação, Serviços)
        // ====================================================================
        $('#ListaContratos').on('change', function () {
            try {
                ...
                // Busca os detalhes do contrato via API
                $.ajax({
                    type: 'get',
                    ...
                    success: function (res) {
                        try {
                            // Debug: Log dos dados retornados para verificação
                            ...
                            // ================================================
                            // RESET: Oculta todas as abas antes de exibir as corretas
                            // ================================================
                            ...
                            // ================================================
                            // TERCEIRIZAÇÃO: Exibe abas de Motoristas, Operadores, Lavadores
                            // Dependendo das flags contratoMotoristas, contratoOperadores, etc.
                            // ================================================
                            if (res.data[0].tipoContrato === 'Terceirização') {
                                // Se o contrato inclui motoristas, exibe a aba correspondente
                                ...
                                // Se o contrato inclui operadores, exibe a aba correspondente
                                if (res.data[0].contratoOperadores === true) {
                                    ...
                                    // Se não houver motoristas, mostra operadores como aba ativa
                                    if (
                                ...
                                // Se o contrato inclui lavadores, exibe a aba correspondente
                                if (res.data[0].contratoLavadores === true) {
                                    ...
                                    // Se não houver motoristas ou operadores, mostra lavadores
                                    if (
        ...
        // ====================================================================
        // EVENT HANDLER: REMOÇÃO DE MOTORISTA DO CONTRATO
        // Delegado no document para funcionar com elementos criados dinamicamente
        // Exibe confirmação via Alerta.Confirmar antes de executar a remoção
        // ====================================================================
        $(document).on('click', '.btn-deletemotoristacontrato', function () {
            try {
                // Obtém o ID do motorista do atributo data-id do botão clicado
                var id = $(this).data('id');
                // Obtém o ID do contrato selecionado no dropdown
                ...
                Alerta.Confirmar(
                    ...
                    try {
                        // Se o usuário confirmou, prossegue com a remoção via API
                        if (willDelete) {
                            // Monta o payload JSON com os IDs necessários
                            ...
                            // Chamada AJAX POST para remover o vínculo
                            $.ajax({
                                url: url,
                                type: 'POST',
                                data: dataToPost,
                                ...
                                success: function (data) {
                                    try {
                                        // Exibe toast de sucesso ou erro conforme retorno
                                        if (data.success) {
                                            AppToast.show(
                                                'Verde',
                                                data.message,
                                            );
                                            // Recarrega a tabela para refletir a alteração
                                            loadTblMotoristas();
                                        } else {
                                            AppToast.show(
                                                'Vermelho',
        ...
        // ====================================================================
        // EVENT HANDLER: REMOÇÃO DE OPERADOR DO CONTRATO
        // Mesmo padrão do handler de motorista - exibe confirmação e remove vínculo
        // ====================================================================
        $(document).on('click', '.btn-deleteoperadorcontrato', function () {
            try {
                // Obtém o ID do operador do atributo data-id do botão clicado
                var id = $(this).data('id');
                // Obtém o ID do contrato selecionado no dropdown
                var contratoid = $('#ListaContratos').val();
                ...
                Alerta.Confirmar(
                    ...
                    try {
                        // Se confirmou, executa a remoção via API
                        if (willDelete) {
        ...
        // ====================================================================
        // EVENT HANDLER: REMOÇÃO DE LAVADOR DO CONTRATO
        // Mesmo padrão do handler de motorista - exibe confirmação e remove vínculo
        // ====================================================================
        $(document).on('click', '.btn-deletelavadorcontrato', function () {
            try {
                // Obtém o ID do lavador do atributo data-id
                var id = $(this).data('id');
                // Obtém o ID do contrato selecionado
                var contratoid = $('#ListaContratos').val();
                ...
                Alerta.Confirmar(
                    ...
                    try {
                        // Se confirmou, executa a remoção
                        if (willDelete) {
                            // Monta o payload JSON
                            var dataToPost = JSON.stringify({
                                LavadorId: id,
        ...
        // ====================================================================
        // EVENT HANDLER: REMOÇÃO DE VEÍCULO DO CONTRATO
        // Usado em contratos do tipo "Locação" para remover veículo vinculado
        // ====================================================================
        $(document).on('click', '.btn-deleteveiculocontrato', function () {
            try {
                // Obtém o ID do veículo do atributo data-id
                var id = $(this).data('id');
                // Obtém o ID do contrato selecionado
                var contratoid = $('#ListaContratos').val();
                ...
                Alerta.Confirmar(
                    ...
                    try {
                        // Se confirmou, executa a remoção via API
                        if (willDelete) {
                            // Monta o payload JSON com os IDs
                            var dataToPost = JSON.stringify({
                                VeiculoId: id,
                                ContratoId: contratoid,
                            });
                            ...
                            // Chamada AJAX POST para remover o vínculo veículo-contrato
                            $.ajax({
                                url: url,
                                type: 'POST',
                                data: dataToPost,
                                ...
                                success: function (data) {
                                    try {
                                        ...
                                        if (data.success) {
                                            ...
                                            // Recarrega a tabela de veículos
                                            loadTblVeiculos();
                                        } else {
                                            AppToast.show(
                                                'Vermelho',
        ...
        // ================================================================
        // TRATAMENTO DE ERRO GLOBAL DO MÓDULO
        // Captura qualquer erro não tratado na inicialização do documento
        // ================================================================
        Alerta.TratamentoErroComLinha(
            'Glosa_001.js',
            'callback@$.ready#0',
