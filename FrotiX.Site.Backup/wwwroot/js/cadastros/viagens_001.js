/**
 * ═══════════════════════════════════════════════════════════════════════════
 * VIAGENS - MÓDULO DE LISTAGEM E GERENCIAMENTO
 * ═══════════════════════════════════════════════════════════════════════════
 * @description Módulo principal de gerenciamento de viagens do FrotiX.
 *              Funcionalidades incluem:
 *              - Listagem de viagens em DataTable com filtros
 *              - Modal de finalização de viagem (km final, combustível, ocorrências)
 *              - Modal de ficha de vistoria
 *              - Modal de impressão
 *              - Cancelamento de viagens
 *              - Integração com Syncfusion (DropDownList, RichTextEditor)
 * @file viagens_001.js
 * @requires jQuery, DataTables, Bootstrap 5, Syncfusion EJ2, Alerta, AppToast
 * @see /Views/Viagem/Index.cshtml
 * ═══════════════════════════════════════════════════════════════════════════
 */
 ...
 * ═══════════════════════════════════════════════════════════════════════════
 * @description Configura componentes e handlers quando o DOM está pronto
 */
$(document).ready(function () {
    try {
        // Workaround: força inicialização dos dropdowns Syncfusion
        // Mostra e esconde popup para garantir que componente está renderizado
        document
        ...
        // Carrega lista de viagens na grid
        ListaTodasViagens();

        /**
         * ─────────────────────────────────────────────────────────────────
         * HANDLER: MODAL FINALIZA VIAGEM
         * ─────────────────────────────────────────────────────────────────
         * @description Abre modal para registrar dados finais da viagem
         * @listens click.btn-modal-finaliza
         */
        $(document).on('click', '.btn-modal-finaliza', function (e) {
            try {
                ...
                /** @type {string} ID da viagem para finalizar */
                var viagemId = $(this).data('id');

                // Preenche campo hidden com ID
        ...
        /**
         * ─────────────────────────────────────────────────────────────────
         * HANDLER: MODAL FICHA DE VISTORIA
         * ─────────────────────────────────────────────────────────────────
         * @description Abre modal para visualizar ficha de vistoria
         * @listens click.btn-modal-ficha
         */
        $(document).on('click', '.btn-modal-ficha', function (e) {
            try {
                e.preventDefault();

                /** @type {string} ID da viagem */
                var viagemId = $(this).data('id');
        ...
        /**
         * ─────────────────────────────────────────────────────────────────
         * HANDLER: MODAL DE IMPRESSÃO
         * ─────────────────────────────────────────────────────────────────
         * @description Abre modal com opções de impressão da viagem
         * @listens click.btn-modal-print
         */
        $(document).on('click', '.btn-modal-print', function (e) {
            try {
                e.preventDefault();

                /** @type {string} ID da viagem para impressão */
                var viagemId = $(this).data('id');
...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * HANDLER: CANCELAMENTO DE VIAGEM
 * ═══════════════════════════════════════════════════════════════════════════
 * @description Handler para cancelar viagem após confirmação do usuário
 * @listens click.btn-cancelar
 */
$(document).on('click', '.btn-cancelar', function () {
    try {
        /** @type {string} ID da viagem a ser cancelada */
        var id = $(this).data('id');
        ...
        Alerta.Confirmar(
            ...
            try {
                ...
                if (willDelete) {
                    try {
                        /** @type {string} JSON com ID da viagem */
                        var dataToPost = JSON.stringify({ ViagemId: id });

                        // Requisição AJAX para cancelar viagem
                        $.ajax({
                            ...
                            /**
                             * @description Callback de sucesso
                             * @param {Object} data - Resposta da API
                             */
                            success: function (data) {
                                try {
                                    if (data.success) {
                                        // Toast verde e recarrega tabela
                                    ...
                                    Alerta.Erro(
                                        ...
                                        'Classe: CancelarViagem | Método: success | Erro: ' +
                                            error.message,
                                    );
                                }
                            },
                            error: function (err) {
                                try {
                                    Alerta.Erro(
                                        '⚠️ Erro Sem Tratamento',
                                        'Classe: CancelarViagem | Método: error | Erro: ' +
                                            err.message,
                                    );
                                } catch (error) {
                                    Alerta.TratamentoErroComLinha(
