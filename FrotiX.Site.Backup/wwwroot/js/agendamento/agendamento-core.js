    ...
    const Recorrencia = {
        ...
        _garantirCalendario: async function () {
            try {
                ...
                // [FIX] Verificar se cultura pt-BR está REALMENTE carregada no Syncfusion
                // Não depender apenas de flag global, mas verificar getCulture()
                let localeCalendario = "en-US";  // Padrão seguro
                if (typeof ej !== "undefined" && ej.base && typeof ej.base.getCulture === "function") {
                    const culturaAtual = ej.base.getCulture();
                    if (culturaAtual === "pt-BR") {
                        localeCalendario = "pt-BR";
                    }
                }
