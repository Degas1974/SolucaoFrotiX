...
<form method="post" enctype="multipart/form-data">
  ...
  <button type="submit" asp-page-handler="Edit" asp-route-id="@Model.Id">
    Atualizar
  </button>
</form>
```
...
## [21/01/2026 14:20] - Padronização: botões e ícone de limpeza de foto

**Descrição**: Ajustado botão de cancelar para classe `btn-vinho` e atualizado ícone da lixeira para FontAwesome Duotone no botão de limpar foto.
...
- ✅ Botões seguem padrão FrotiX (btn-azul/btn-vinho)
- ✅ Ícone de ação alinhado ao padrão Duotone
...
- Pages/Usuarios/Upsert.cshtml - Substituição de `btn-ftx-fechar` por `btn-vinho` em botão de modal
...
## [11/01/2026 16:30] - Correção: Avatar não exibido e ordenação incorreta na lista

**Descrição**:
Corrigidos dois problemas na listagem de usuários:

1. Avatar não era exibido mesmo com foto gravada
2. Ordenação alfabética não funcionava corretamente

**Problema 1 - Avatar não exibido**:

- **Sintoma**: Usuários com foto gravada mostravam apenas o ícone azul de usuário sem foto
- **Causa Raiz**: O CSS define `.ftx-avatar-img` com `opacity: 0` e `visibility: hidden` por padrão. A imagem só fica visível se tiver a classe `is-visible` ou estiver dentro de um container `.ftx-avatar`.
- **Solução**: Adicionada classe `is-visible` à imagem e envolta em container `.ftx-avatar`

**Problema 2 - Ordenação incorreta**:

- **Sintoma**: Usuários com foto apareciam fora da ordem alfabética
- **Causa Raiz**: O DataTable estava ordenando pelo HTML renderizado (com avatar) em vez do texto puro do nome
- **Solução**: Adicionado tratamento para `type === 'sort' || type === 'filter'` retornando apenas o texto
...
**Código Adicionado (Ordenação)**:

```javascript
// Para ordenação e filtro, usar apenas o texto
...
- `wwwroot/js/cadastros/usuario-index.js` (linhas 165-200)

**Impacto**:

- ✅ Avatar exibido corretamente para usuários com foto
- ✅ Ordenação alfabética funcionando
- ✅ Filtro de busca funcionando com nome puro
...
## [11/01/2026 15:00] - Correção Crítica: Campos obrigatórios do Identity não eram preenchidos

**Descrição**:
Corrigido bug crítico onde novos usuários eram criados com campos NULL essenciais do ASP.NET Core Identity, causando problemas de exibição e ordenação na lista.

**Problema**:

- **Sintoma**:
  - Avatar não aparecia na lista de usuários (mesmo tendo foto salva)
  - Registros fora da ordenação alfabética
  - Usuários novos apareciam como "Inativo" mesmo sendo criados como "Ativo"
- **Causa Raiz**: O `UserManager<IdentityUser>` (não `UserManager<AspNetUsers>`) não salva propriedades customizadas. O código original tentava usar UserManager, mas as propriedades customizadas (Status, Foto, etc.) não eram gravadas corretamente.
- **Campos afetados (estavam NULL no banco)**:
  - `NormalizedUserName`
  - `NormalizedEmail`
  - `PasswordHash`
  - `SecurityStamp`
  - `ConcurrencyStamp`
  - `Status` (era False em vez de True)
  - `UsuarioIdAlteracao`

**Solução Aplicada**:

1. **OnPostSubmit (Criação)**:
   - Forçado `Status = true` (não usar `??` porque `false ?? true = false`)
   - Adicionado `UsuarioIdAlteracao` com ID do usuário logado
   - Todos os campos Identity já eram preenchidos diretamente via `_unitOfWork`

2. **OnPostEdit (Edição)**:
   - Adicionado preenchimento de `NormalizedUserName = UserName?.ToUpper()`
   - Adicionado preenchimento de `NormalizedEmail = Email?.ToUpper()`
   - Adicionado `UsuarioIdAlteracao` com ID do usuário logado
   - Preservados campos do usuário existente que não devem ser alterados:
     - `PasswordHash`, `SecurityStamp`, `ConcurrencyStamp`
     - `Criacao`, `UltimoLogin`
     - `EmailConfirmed`, `PhoneNumberConfirmed`
     - `TwoFactorEnabled`, `LockoutEnabled`, `AccessFailedCount`

**Código Antes (INCORRETO - Status)**:

```csharp
bool statusUsuario = UsuarioObj.AspNetUsers.Status ?? true;
// Problema: false ?? true = false (não avalia o segundo operando se o primeiro não é null)
```

**Código Depois (CORRETO - Status)**:
...
- `Pages/Usuarios/Upsert.cshtml.cs` (linhas 223-254, 369-390)

**Impacto**:

- ✅ Novos usuários agora têm todos os campos obrigatórios preenchidos
- ✅ Avatar aparece corretamente na lista (NormalizedUserName não é mais NULL)
- ✅ Registros aparecem na ordenação alfabética correta
- ✅ Novos usuários aparecem como "Ativo" (Status = true forçado)
- ✅ Edição preserva campos existentes e preenche os obrigatórios faltantes
- ✅ UsuarioIdAlteracao registra quem criou/alterou o usuário

**Teste Sugerido**:

1. Criar novo usuário com todos os campos
2. Verificar no banco que `NormalizedUserName`, `NormalizedEmail`, `Status=1`, `UsuarioIdAlteracao` estão preenchidos
3. Verificar na lista que avatar aparece e está ordenado corretamente
4. Editar um usuário existente que tinha campos NULL
5. Verificar que campos obrigatórios foram preenchidos após edição
...
```html
<form method="post" asp-action="Upsert" enctype="multipart/form-data"></form>
```
