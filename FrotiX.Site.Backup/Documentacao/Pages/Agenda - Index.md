...
> **Ultima Atualizacao**: 23/01/2026 10:00
> **Versao Atual**: 5.4
...
## [23/01/2026 10:00] - Correção: Erro percentSign ao Fechar Modal

**Descricao**: Corrigido erro `TypeError: Cannot read properties of undefined (reading 'percentSign')` que ocorria ao fechar o modal de Novo Agendamento sem gravar.

**Problema Identificado**:

- Ao fechar o modal sem gravar, componentes Syncfusion (NumericTextBox, etc.) tentavam formatar valores
- Localização pt-BR do Syncfusion não estava sendo carregada automaticamente
- Componentes tentavam acessar `percentSign` da cultura, mas ela não existia
- Erro: `TypeError: Cannot read properties of undefined (reading 'percentSign')`

**Causa Raiz**:

- Arquivo `localization-init.js` era carregado mas a função `loadSyncfusionLocalization()` não era chamada
- Componentes Syncfusion eram renderizados ANTES da localização estar disponível
- Ao fechar modal ou limpar componentes, tentavam formatar usando cultura inexistente

**Solução Implementada**:

Adicionado script inline logo após `localization-init.js` para chamar a função imediatamente:
    ...
    (async function () {
        try {
            if (typeof window.loadSyncfusionLocalization === 'function') {
                ...
                console.log('[Agenda/Index] ✅ Localização Syncfusion pt-BR carregada com sucesso');
            } else {
                console.warn('[Agenda/Index] ⚠️ Função loadSyncfusionLocalization não encontrada');
            }
        } catch (error) {
            console.error('[Agenda/Index] ❌ Erro ao carregar localização Syncfusion:', error);
        }
    })();
</script>
```
...
- `Pages/Agenda/Index.cshtml` (linhas 2118-2139)

**Impacto**:

- Localização pt-BR carregada ANTES de qualquer componente Syncfusion ser usado
- Elimina erro ao fechar modal sem gravar
- Componentes NumericTextBox, Calendar e outros agora têm acesso correto a formatação pt-BR
- Melhora estabilidade geral da página
