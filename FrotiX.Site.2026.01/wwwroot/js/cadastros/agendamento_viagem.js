 ...
 * ╠══════════════════════════════════════════════════════════════════════════╣
 * ║ ❓ POR QUE EXISTO?                                                       ║
 * ║    Calendário de agendamento de viagens com FullCalendar.                ║
 * ║    • Visualização mensal/semanal/diária de viagens                       ║
 * ║    • Drag & drop para reagendamento                                      ║
 * ║    • Integração com modal de criação/edição                              ║
 * ║    • Validação de conflitos de motorista/veículo                         ║
 * ║    • Recorrência de viagens (semanal, mensal)                            ║
 * ║                                                                          ║
 * ║ 🔗 RELEVÂNCIA: Crítica (Core - Agendamento)                              ║
 * ╚══════════════════════════════════════════════════════════════════════════╝
 */
...
function criarAgendamentoNovo() {
    try {
        ...
        const txtDataInicial =
            document.getElementById('txtDataInicial')?.ej2_instances?.[0];
        const txtDataFinal =
        ...
        const requisitanteId =
            document.getElementById('lstRequisitante')?.ej2_instances?.[0]
                ?.value ?? null;
        const destino =
        ...
        if (Array.isArray(eventosVal) && eventosVal.length)
            eventoId = eventosVal[0];

        const dataInicialDate = txtDataInicial?.value
            ? new Date(txtDataInicial.value)
            : null;
        const dataInicialStr = dataInicialDate
            ? toDateOnlyString(dataInicialDate)
            : null;
        const horaInicioTexto = $('#txtHoraInicial').val();
        const horaInicioLocal =
            dataInicialDate && horaInicioTexto
                ? toLocalDateTimeString(dataInicialDate, horaInicioTexto)
                : null;
        ...
        const dataFinalStr = dataFinalDate
            ? toDateOnlyString(dataFinalDate)
            : null;
        ...
        const dataFinalRecorrenciaEl = document.getElementById(
            'txtdataFinalRecorrencia',
        )?.ej2_instances?.[0];
        const dataFinalRecorrencia = dataFinalRecorrenciaEl?.value
            ? toDateOnlyString(new Date(dataFinalRecorrenciaEl.value))
            : null;
        ...
        const diaMesRecorrencia = parseIntSafe(
            $('#txtDiaMesRecorrencia').val(),
        );

        let datasSelecionadas = null;
        if (
            Array.isArray(window.selectedDates) &&
            window.selectedDates.length
        ) {
            datasSelecionadas = window.selectedDates
                .map((x) => {
                    try {
...
async function calendarEvents(fetchInfo, successCallback, failureCallback) {
    try {
        const resp = await fetch(
            `/api/Agenda/Eventos?start=${fetchInfo.startStr}&end=${fetchInfo.endStr}`,
        );
        ...
        try {
            ...
            try {
                ...
                dataInicial = moment(
                    ...
                    .toISOString()
                    .split('T')[0];
                const periodoRecorrente =
                    document.getElementById('lstPeriodos').ej2_instances[0]
                        .value;
                if (!viagemId) {
                    if (periodoRecorrente === null) {
                        try {
                            ...
                            Alerta.TratamentoErroComLinha(
                                'agendamento_viagem.js',
                                'criarAgendamentoUnico',
                                error,
                            );
                        }
                    } else if (periodoRecorrente === 'M') {
                        const datasRecorrentes =
                            ajustarDataInicialRecorrente(periodoRecorrente);
                    ...
                    viagemId != null &&
                    viagemId !== '' &&
                    $('#btnConfirma').text() === 'Registra Viagem'
                ) {
                    transformandoEmViagem = true;

                    try {
                        const agendamentoUnicoAlterado =
                            await recuperarViagemEdicao(viagemId);
                        let objViagem = criarAgendamentoViagem(
                        ...
                        if (data.success) {
                            ...
                            Alerta.Erro(
                                'Erro ao criar Agendamento',
                                'Não foi possível criar a Viagem com os dados informados',
                            );
                        }
                    } catch (error) {
                        Alerta.TratamentoErroComLinha(
                                    ...
                                    try {
                                        ...
                                        if (objViagem.recorrente === 'S') {
                                            ...
                                            if (confirmacao) {
                                                ...
                                                await editarAgendamentoRecorrente(
                                                    viagemId,
                                                    true,
                                                    objViagem.dataInicial,
                                                    objViagem.recorrenciaViagemId,
                                                    editarTodosRecorrentes,
                                                );
                                            } else {
            ...
            try {
                $.ajax({
                    ...
                    success: function (data) {
                        try {
                            console.log(
                                'Requisição AJAX bem-sucedida, dados recebidos:',
                                data,
                            );
                            resolve(data);
                        } catch (error) {
...
async function editarAgendamentoRecorrente(
    viagemId,
    editaTodos,
            ...
            if (editaTodos) {
                if (
                    ...
                    const [primeiroDaSerie = {}] =
                        await obterAgendamentosRecorrenteInicial(viagemId);
                    let objViagem = criarAgendamentoEdicao(primeiroDaSerie);

                    if ('DataInicial' in objViagem)
                        delete objViagem.DataInicial;
                ...
                const agendamentos =
                    await obterAgendamentosRecorrentes(recorrenciaViagemId);
                for (const agendamentoRecorrente of agendamentos) {
                    if (
                        ...
                        let objViagem = criarAgendamentoEdicao(
...
async function handleRecurrence(periodoRecorrente, datasRecorrentes) {
    try {
        if (!datasRecorrentes || datasRecorrentes.length === 0) {
            console.error(
                'Nenhuma data inicial válida retornada para o período.',
            );
            return;
        }

        let primeiroAgendamento = criarAgendamento(
            null,
            null,
            datasRecorrentes[0],
        );
        let agendamentoObj;
        try {
            agendamentoObj = await enviarNovoAgendamento(
                primeiroAgendamento,
                datasRecorrentes.length === 1,
            );
            if (
                !agendamentoObj ||
                !agendamentoObj.novaViagem ||
        ...
        if (datasRecorrentes.length > 1) {
            for (let i = 1; i < datasRecorrentes.length; i++) {
                const agendamentoSubsequente = criarAgendamento(
                    null,
                    agendamentoObj.novaViagem.viagemId,
                    datasRecorrentes[i],
                );
                try {
                    await enviarNovoAgendamento(
                        agendamentoSubsequente,
                        i === datasRecorrentes.length - 1,
                    );
                } catch (error) {
                    Alerta.TratamentoErroComLinha(
                        'agendamento_viagem.js',
    ...
    try {
        try {
            ...
            if (!objViagem.operacaoBemSucedida) {
                console.error(
                    'Erro ao criar novo agendamento: operação não bem-sucedida',
                    objViagem,
                );
...
async function enviarAgendamentoComOpcao(
    viagemId,
    editarTodos,
    editarProximos,
    dataInicial = null,
...
async function obterEDefinirDatasCalendario(viagem, viagemIdRecorrente) {
    try {
        try {
            ...
            const datasIniciais = await getDatasIniciais(
                viagem,
                viagemIdRecorrente,
            );
...
async function getDatasIniciais(viagemId, recorrenciaViagemId) {
    try {
        console.log(
            'Chamando getDatasIniciais com viagemId:',
            viagemId_AJAX,
            'e recorrenciaViagemId:',
            recorrenciaViagemId_AJAX,
        );
        try {
            return new Promise((resolve, reject) => {
                try {
...
function atualizarCalendarioExistente(datas) {
    try {
        ...
        let calendarObj = document.getElementById('calDatasSelecionadas')
            .ej2_instances[0];
        if (calendarObj) {
...
async function enviarAgendamento(agendamento) {
    try {
        if (isSubmitting) {
            console.warn(
                'Tentativa de enviar enquanto outra requisição está em andamento.',
            );
            return;
        }
        isSubmitting = true;
        ...
        try {
            ...
            if (response?.novaViagem?.operacaoBemSucedida === true) {
                ...
                console.error(
                    'Erro ao enviar agendamento: operação não bem-sucedida.',
                    response,
                );
                throw new Error(
                    'Erro ao criar agendamento. Operação não bem-sucedida.',
                );
            }
...
function ajustarDataInicialRecorrente(tipoRecorrencia) {
    try {
        ...
        let dataAtual =
            document.getElementById('txtDataInicial')?.ej2_instances?.[0]
                ?.value;
        const dataFinal = document.getElementById('txtFinalRecorrencia')
        ...
        const dataFinalFormatada = moment(dataFinal)
            .toISOString()
            .split('T')[0];
        let diasSelecionados =
            document.getElementById('lstDias')?.ej2_instances?.[0]?.value || [];
        if (tipoRecorrencia === 'M') {
            diasSelecionados = [].concat(
                document.getElementById('lstDiasMes')?.ej2_instances?.[0]
                    ?.value || [],
            );
        }
        ...
        if (tipoRecorrencia === 'D') {
            ...
            gerarRecorrenciaPorPeriodo(
                tipoRecorrencia,
                dataAtual,
                dataFinalFormatada,
                diasSelecionadosIndex,
                datas,
            );
        }
        return datas.length > 0 ? datas : null;
...
function gerarRecorrenciaVariada(datas) {
    try {
        ...
        if (
            !calendarObj ||
            !calendarObj.values ||
            calendarObj.values.length === 0
        ) {
            console.error(
                "Nenhuma data selecionada no calendário para recorrência do tipo 'V'.",
            );
            return;
        }
        ...
        while (dataAtual.isSameOrBefore(dataFinal)) {
            diasSelecionados.forEach((dia) => {
                try {
                    ...
                    if (
                        proximaData.isSameOrBefore(dataFinal) &&
                        proximaData.isSameOrAfter(dataAtual.startOf('month'))
                    ) {
...
function gerarRecorrenciaDiaria(dataAtual, dataFinal, datas) {
    try {
        ...
        Alerta.TratamentoErroComLinha(
            'agendamento_viagem.js',
            'gerarRecorrenciaDiaria',
            error,
        );
    }
}

function gerarRecorrenciaPorPeriodo(
    tipoRecorrencia,
    dataAtual,
    dataFinal,
    diasSelecionadosIndex,
        ...
        while (dataAtual.isSameOrBefore(dataFinal)) {
            diasSelecionadosIndex.forEach((diaSelecionado) => {
                try {
                    ...
                    if (
                        proximaData.isSameOrBefore(dataFinal) &&
                        !datas.includes(proximaData.format('YYYY-MM-DD'))
                    ) {
                        datas.push(proximaData.format('YYYY-MM-DD'));
                    }
                } catch (error) {
...
function exibirMensagemSucesso() {
    try {
        AppToast.show(
            'Verde',
            'Todos os agendamentos foram criados com sucesso',
            3000,
        );
        Alerta.Sucesso(
            'Agendamento criado com sucesso',
...
async function ValidaCampos(viagemId) {
    try {
        ...
        const valDataInicial =
            document.getElementById('txtDataInicial').ej2_instances[0].value;
        const lstDdataInicial =
            document.getElementById('txtDataInicial').ej2_instances[0];
        if (
            !valDataInicial ||
            !moment(valDataInicial).isValid() ||
            valDataInicial === null
        ) {
        ...
        if (
            viagemId != null &&
            viagemId !== '' &&
            $('#btnConfirma').text() !== 'Edita Agendamento'
        ) {
            if (
                document.getElementById('txtNoFichaVistoria').value === '' ||
                document.getElementById('txtNoFichaVistoria').value === null
            ) {
                await Alerta.Erro(
                    'Informação Ausente',
        ...
        if (
            ...
            const evento =
                document.getElementById('lstEventos').ej2_instances[0].value;
        ...
        if (transformandoEmViagem === false) {
            ...
            if (
                (periodo === 'S' || periodo === 'Q' || periodo === 'M') &&
                (document.getElementById('lstDias').ej2_instances[0].value ===
                    '' ||
                    document.getElementById('lstDias').ej2_instances[0]
                        .value === null)
            ) {
                await Alerta.Erro(
                    'Informação Ausente',
                    'Se o período foi escolhido como semanal, quinzenal ou mensal, você precisa escolher os Dias da Semana',
                );
                return false;
            }
        }
        ...
        if (
            (periodo === 'D' ||
                ...
                periodo === 'Q' ||
                periodo === 'M') &&
            (document.getElementById('txtFinalRecorrencia').ej2_instances[0]
                .value === '' ||
                document.getElementById('txtFinalRecorrencia').ej2_instances[0]
                    .value === null)
        ) {
...
function criarAgendamento(viagemId, viagemIdRecorrente, dataInicial) {
    try {
        ...
        const dataFinalInput = document.getElementById('txtFinalRecorrencia')
            .ej2_instances[0].value;
        const momentDate = moment(dataFinalInput);
        const dataFinalRecorrencia = momentDate.isValid()
        ...
        const dataInicialFormatada = moment(dataInicial).isValid()
...
function criarAgendamentoUnico() {
    try {
        ...
        let eventoId = '';
        if (
            document.getElementById('lstEventos').ej2_instances[0].value != null
        ) {
            let valor =
                document.getElementById('lstEventos').ej2_instances[0].value;
            console.log(valor);
        ...
        let setorId =
            document.getElementById('ddtSetor').ej2_instances[0].value[0];
        let ramal = $('#txtRamalRequisitante').val();
        let requisitanteId =
            document.getElementById('lstRequisitante').ej2_instances[0].value;
...
function criarAgendamentoEdicao(agendamentoOriginal) {
    try {
        ...
        const dataBaseOriginal = agendamentoOriginal?.dataInicial
            ? new Date(agendamentoOriginal.dataInicial)
            : null;

        const horaInicioTexto = $('#txtHoraInicial').val();
        const horaInicioLocal =
            dataBaseOriginal && horaInicioTexto
                ? toLocalDateTimeString(dataBaseOriginal, horaInicioTexto)
                : null;
        ...
        const statusAgendamento =
            agendamentoOriginal?.statusAgendamento ?? true;
...
function criarAgendamentoViagem(agendamentoUnicoAlterado) {
    try {
        ...
        if (
            ...
            eventoId =
                document.getElementById('lstEventos').ej2_instances[0].value[0];
        } else {
            eventoId = null;
        }
        let setorId =
        ...
        let dataFinal = '';
        if (
            document.getElementById('txtDataFinal').ej2_instances[0].value ===
                null ||
            document.getElementById('txtDataFinal').ej2_instances[0].value ===
                undefined
        ) {
...
$(document).ready(function () {
    try {
        ...
        // Handler para fechar
        $('#btnFecharAccordionRequisitante')
            .off('click')
            .on('click', function (e) {
                e.preventDefault();
                e.stopPropagation();
                ...
                accordionContainer.slideUp(300, function () {
                    ...
                    try {
                        var setores = document.getElementById(
        ...
        var dropDownListRecorrente = new ej.dropdowns.DropDownList({
            ...
            change: function (e) {
                try {
                    ...
                    var listBox =
                        document.getElementById('lstDiasCalendario')
                            .ej2_instances[0];
                    listBox.dataSource = [];
                    document.getElementById('itensBadge').textContent = 0;
                    if (selectedValue === 'S') {
                        if (
                            document.getElementById('divTxtPeriodo').style
                                .display != 'block'
                        ) {
                            document.getElementById(
                                'divPeriodo',
                            ...
                            document.getElementById('divDias').style.display =
                                'none';
                            document.getElementById('divDiaMes').style.display =
                                'none';
                            document.getElementById(
                                'divFinalRecorrencia',
                            ).style.display = 'none';
                            document.getElementById(
                                'divFinalFalsoRecorrencia',
                            ).style.display = 'none';
                            var calendarContainer =
                                document.getElementById('calendarContainer');
                            calendarContainer.style.display = 'none';
                            var listboxContainer =
                                document.getElementById('listboxContainer');
                            listboxContainer.style.display = 'none';
                        }
                    } else {
                        document.getElementById('divPeriodo').style.display =
                            'none';
                        document.getElementById('divDias').style.display =
                            'none';
                        document.getElementById('divDiaMes').style.display =
        ...
        let dialogRecorrencia = new ej.popups.Dialog({
            header:
                ...
                '</div>',
            content:
                '<div style="font-size: 16px; color: #333;">A data final não pode ser maior que 365 dias após a data inicial.</div>',
            position: {
                X: 'center',
                Y: 'center',
            },
        ...
        function syncListBoxAndBadges() {
            try {
                ...
                const totalItems =
                    window.listBox && typeof listBox.getDataList === 'function'
                        ? listBox.getDataList().length
        ...
        function handleCalendarChange(args) {
            try {
                const selectedDatesArray = Array.isArray(args.values)
                    ? args.values
                    : args.value
                      ? [args.value]
                      : [];
        ...
        const calendar = new ej.calendars.Calendar({
            ...
            change: function (args) {
                try {
                    ...
                    listBox.dataBind();
                    const totalItems = listBox.getDataList().length;
                    document.getElementById('itensBadge').textContent =
                        totalItems;
                    document.getElementById('itensBadgeHTML').textContent =
                        totalItems;
                } catch (error) {
                    Alerta.TratamentoErroComLinha(
                        'agendamento_viagem.js',
            ...
            change: function (args) {
                try {
                    ...
                    if (itemValue && multiSelect.value.includes(itemValue)) {
                        multiSelect.value = multiSelect.value.filter(
                            (value) => {
                                try {
        ...
        document
            .getElementById('lstRecorrente')
            .ej2_instances[0].setProperties({ height: '200px' });
        document
            .getElementById('lstPeriodos')
            .ej2_instances[0].setProperties({ height: '200px' });
        document
            .getElementById('lstDias')
            .ej2_instances[0].setProperties({ height: '200px' });
        document
            .getElementById('txtFinalRecorrencia')
            .ej2_instances[0].setProperties({ height: '200px' });

        document.getElementById('divDias').style.display = 'none';
        document.getElementById('divFinalRecorrencia').style.display = 'none';
        document.getElementById('divFinalFalsoRecorrencia').style.display =
            'none';
...
function inicializarSistemaEventoCompleto() {
    try {
        ...
        if (
            lstFinalidadeElement &&
            lstFinalidadeElement.ej2_instances &&
            lstFinalidadeElement.ej2_instances[0]
        ) {
        ...
        if (btnFecharAccordion) {
            ...
            btnFecharAccordion.parentNode.replaceChild(
                novoBtnFechar,
                btnFecharAccordion,
            );
        ...
        if (btnInserirEvento) {
            ...
            btnInserirEvento.parentNode.replaceChild(
                novoBtnInserir,
                btnInserirEvento,
            );
...
function controlarCardEvento(finalidade) {
    try {
        ...
        if (Array.isArray(finalidade)) {
            mostrarCard = finalidade.some(
                (f) => f && (f === 'Evento' || f.toLowerCase() === 'evento'),
            );
        } else if (finalidade) {
            mostrarCard =
                finalidade === 'Evento' ||
...
function limparCamposEventoFormulario() {
    try {
        ...
        // Componentes EJ2
        const componentesEJ2 = ['lstRequisitanteEvento', 'ddtSetorEvento'];
        ...
        setTimeout(function () {
            try {
                ...
                if (viagem.eventoId != null || viagem.finalidade === 'Evento') {
                    const lstFinalidade =
                        document.getElementById('lstFinalidade');
                    if (
                        lstFinalidade &&
                        lstFinalidade.ej2_instances &&
                        lstFinalidade.ej2_instances[0]
                    ) {
                        const finalidadeAtual =
...
function inicializarCamposModal() {
    try {
        ...
        if (
            lstEvento &&
            lstEvento.ej2_instances &&
            lstEvento.ej2_instances[0]
        ) {
...
function configurarModoNovoAgendamento(dataClicada, horaClicada) {
    ...
    $('#btnConfirma').html(
        "<i class='fa fa-save' aria-hidden='true'></i>Cria Agendamento",
    );
...
function preencherDataInicial(dataClicada) {
    try {
        ...
        if (dataClicada) {
            ...
            console.log(
                'Data preenchida:',
                moment(dataClicada).format('DD/MM/YYYY'),
            );
        } else {
            datePickerInstance.value = moment().toDate();
            console.log(
                'Data preenchida (atual):',
...
function preencherCamposBasicos(viagem) {
    try {
        ...
        if (
            ...
            datePicker.ej2_instances[0].value = moment(
                viagem.dataInicial,
            ).toDate();
        ...
        document.getElementById('txtHoraInicial').value =
            viagem.horaInicio.substring(11, 16);
        $('#txtHoraInicial').attr('type', 'time');
        ...
        if (viagem.requisitanteId != null) {
            ...
            if (
                ...
                lstRequisitante.ej2_instances[0]
            ) {
        ...
        // Combustível
        if (
            viagem.combustivelInicial != null &&
            viagem.combustivelInicial !== ''
        ) {
            const ddtCombInicial = document.getElementById(
                'ddtCombustivelInicial',
            );
            if (
                ddtCombInicial &&
                ddtCombInicial.ej2_instances &&
                ddtCombInicial.ej2_instances[0]
            ) {
                ddtCombInicial.ej2_instances[0].value = [
                    viagem.combustivelInicial,
                ];
            }
        }
        ...
        if (viagem.combustivelFinal != null && viagem.combustivelFinal !== '') {
            ...
            if (
                ddtCombFinal &&
                ddtCombFinal.ej2_instances &&
                ddtCombFinal.ej2_instances[0]
            ) {
...
function exibirLabelsUsuarios(viagem) {
    ...
    // Cancelado por
    if (
        viagem.usuarioIdCancelamento != null &&
        viagem.dataCancelamento != null
    ) {
        exibirLabelUsuario(
...
function configurarRecorrencia(viagem) {
    try {
        ...
        if (
            lstRecorrente &&
            lstRecorrente.ej2_instances &&
            lstRecorrente.ej2_instances[0]
        ) {
        ...
        if (['D', 'S', 'Q', 'M'].includes(viagem.intervalo)) {
            const txtdataFinalRecorrencia = document.getElementById(
                'txtdataFinalRecorrencia',
            );
            if (txtdataFinalRecorrencia) {
                txtdataFinalRecorrencia.disabled = true;
                txtdataFinalRecorrencia.value = moment(
                    viagem.dataFinalRecorrencia,
                ).format('DD/MM/YYYY');
            }
            ...
            const divFinalFalsoRecorrencia = document.getElementById(
                'divFinalFalsoRecorrencia',
            );

            if (divFinalRecorrencia) divFinalRecorrencia.style.display = 'none';
            if (divFinalFalsoRecorrencia)
                divFinalFalsoRecorrencia.style.display = 'block';
        } else {
            const divFinalRecorrencia = document.getElementById(
                'divFinalRecorrencia',
            );
            ...
            if (divFinalRecorrencia) divFinalRecorrencia.style.display = 'none';
            if (divFinalFalsoRecorrencia)
                divFinalFalsoRecorrencia.style.display = 'none';
        }
    } catch (error) {
...
function determinarEstadoViagem(viagem) {
    ...
    if (viagem.status === 'Cancelada' && viagem.statusAgendamento === false)
        return 'CANCELADA';
...
function configurarAgendamentoAtivo(viagem) {
    ...
    $('#btnConfirma').html(
        "<i class='fa fa-save' aria-hidden='true'></i>Edita Agendamento",
    );
...
function exibirCamposViagem(viagem) {
    try {
        // divNoFichaVistoria removido - agora usa botão ao lado do Destino (21/01/2026)
        const camposViagem = [
            'divDataFinal',
            'divHoraFinal',
            'divDuracao',
        ...
        const txtNoFichaVistoria =
            document.getElementById('txtNoFichaVistoria');
        ...
        calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            select: function (info) {
                try {
                    ...
                    const startArredondado = moment(start)
                        .set({
                    ...
                    const startStr = moment(startArredondado).format(
                        'YYYY-MM-DD HH:mm:ss',
                    );
                    const dataStr =
                        moment(startArredondado).format('YYYY-MM-DD');
                    ...
                    if (
                        elData &&
                        elData.ej2_instances &&
                        elData.ej2_instances.length
                    ) {
                    ...
                    if (
                        elHora &&
                        elHora.ej2_instances &&
                        elHora.ej2_instances.length
                    ) {
...
function hideAccordionRequisitante() {
    try {
        ...
        if (
            ddSetor &&
            Array.isArray(ddSetor.ej2_instances) &&
            ddSetor.ej2_instances[0]
        ) {
...
document.addEventListener('click', function (event) {
    try {
        ...
        if (
            !accordionElement.contains(event.target) &&
            event.target !== btnRequisitante
        ) {
            hideAccordionRequisitante();
        }
    } catch (error) {
        Alerta.TratamentoErroComLinha(
            'agendamento_viagem.js',
            'addEventListener:click_callback',
            error,
        );
    }
});
...
var accordionElementRequisitante = document.getElementById(
    'accordionRequisitante',
);
accordionElementRequisitante.style.display = 'none';
toggleAccordionBtnRequisitante.addEventListener('click', function () {
    try {
        var displayValue = window.getComputedStyle(
            accordionElementRequisitante,
        ).display;
        ...
        try {
            ...
            if (txtDataInicial && txtFinalRecorrencia) {
                ...
                if (diferencaDias > 365) {
                    Alerta.Warning(
                        'Atenção',
                        'A data final não pode ser maior que 365 dias após a data inicial',
                    );
                    document.getElementById('txtFinalRecorrencia').value = '';
                }
            }
            ...
            Alerta.TratamentoErroComLinha(
                'agendamento_viagem.js',
                'addEventListener:focusout_callback',
                error,
            );
        }
    });
                ...
                success: function (response) {
                    try {
                        if (response.data.status == 'Cancelada') {
                            ...
                            relatorioAsString =
                                response.data.finalidade != 'Evento'
                                    ? 'FichaCancelada.trdp'
    ...
    .on('hide.bs.modal', function (event) {
        try {
            ...
            $('#ReportContainer').append(
                "<div id='fichaReport' style='width:100%' class='pb-3'> Carregando... </div>",
            );
            $(document.body).removeClass('modal-open');
            $('.modal-backdrop').remove();
            $(document.body).css('overflow', '');
...
$('#btnViagem').click(function (event) {
    try {
        ...
        $('#btnViagem').hide();
        $('#btnConfirma').html(
            "<i class='fa fa-save' aria-hidden='true'></i>Registra Viagem",
        );
        // divNoFichaVistoria removido - agora usa botão ao lado do Destino (21/01/2026)
        ...
        document.getElementById('divCombustivelInicial').style.display =
            'block';
...
$('#btnApaga').click(async function (event) {
    try {
        var viagemId = document.getElementById('txtViagemId').value;
        var recorrenciaViagemId = document.getElementById(
            'txtRecorrenciaViagemId',
        ).value;
        var rteDescricao =
            document.getElementById('rteDescricao').ej2_instances[0];
        let titulo = '';
        if (
            (recorrenciaViagemId != null && recorrenciaViagemId != '') ||
            recorrenciaViagemId != '00000000-0000-0000-0000-000000000000'
        ) {
            titulo =
                'Você gostaria de apagar todos os agendamentos recorrentes? Ou somente o atual?';
        } else {
        ...
        if (confirmacao) {
            try {
                ...
                AppToast.show(
                    'Verde',
                    'Todos os agendamentos foram excluídos com sucesso!',
                    3000,
                );
                $('#modalViagens').modal('hide');
                $(document.body).removeClass('modal-open');
...
$('#btnCancela').click(async function (event) {
    try {
        ...
        if (isRecorrente) {
            titulo =
                'Você gostaria de cancelar todos os agendamentos recorrentes? Ou somente o atual?';
        } else {
        ...
        if (confirmacao && isRecorrente) {
            ...
            try {
                ...
                const agendamentosRecorrentes =
                    await obterAgendamentosRecorrentes(recorrenciaId);
                ...
                if (cancelados === 1) {
                    ...
                    AppToast.show(
                        'Verde',
                        `${cancelados} agendamentos foram cancelados com sucesso!`,
                        2000,
                    );
                }
            } catch (error) {
            ...
            if (isRecorrente) {
                console.log(
                    '📋 ENTRANDO NO BLOCO: Cancelar Apenas o Atual (de uma série recorrente)',
                );
            } else {
...
async function cancelarAgendamento(viagemId, descricao, mostrarToast = true) {
    try {
        console.log(
            `🎯 cancelarAgendamento - viagemId: ${viagemId}, mostrarToast: ${mostrarToast}`,
        );
                ...
                success: function (data) {
                    try {
                        if (data.success) {
                            ...
                            if (mostrarToast) {
                                console.log(
                                    '✅ Mostrando toast da função cancelarAgendamento',
                                );
                                AppToast.show(
                                    'Verde',
                                    'O agendamento foi cancelado com sucesso!',
...
$('#txtKmInicial').focusout(function () {
    try {
        ...
        if (kmInicial > kmFinal) {
            ...
            Alerta.Erro(
                'Erro na Quilometragem',
                'A quilometragem inicial deve ser menor que a final',
            );
        }
    } catch (error) {
        Alerta.TratamentoErroComLinha(
...
$('#txtKmInicial').focusout(function () {
    try {
        ...
        if (kmInicial != kmAtual) {
            Alerta.Erro(
                'Erro na Quilometragem',
                'A quilometragem inicial não confere com a atual',
            );
            return;
        }
    } catch (error) {
...
function PreencheListaSetores() {
    try {
        $.ajax({
            ...
            success: function (res) {
                try {
                    ...
                    for (var i = 1; i < res.data.length; ++i) {
                        console.log(
                            res.data[i].requisitanteId +
                                res.data[i].requisitante,
                        );
        ...
        document
            .getElementById('ddtSetorRequisitante')
...
$('#btnInserirEvento').click(async function (e) {
    try {
        ...
        var setores = document.getElementById('lstSetorRequisitanteEvento')
            .ej2_instances[0];
                    ...
                    try {
                        if (data.success) {
                            ...
                            var dropdownInstance =
                                document.getElementById('lstEventos')
                                    .ej2_instances[0];
                            ...
                            let updatedDataSource = Array.isArray(
                                dropdownInstance.dataSource,
                            )
                                ? dropdownInstance.dataSource
                                : [];
...
document.getElementById('btnFecharAccordionRequisitante').onclick =
    function () {
        try {
            hideAccordionRequisitante();
        } catch (error) {
...
document.addEventListener('DOMContentLoaded', function () {
    try {
        try {
            ...
            window.removeDate = function (timestamp) {
                try {
                    ...
                    const calendarObj = document.getElementById(
                        'calDatasSelecionadas',
                    ).ej2_instances[0];
                    ...
                    currentSelectedDates = currentSelectedDates.filter(
                        (date) => {
                            try {
                                const normalizedDate = new Date(date).setHours(
                                    0,
                                    0,
                                    0,
                                    0,
        ...
        Alerta.TratamentoErroComLinha(
            'agendamento_viagem.js',
            'addEventListener:DOMContentLoaded_callback',
            error,
        );
    }
});
...
rteDescricao.addEventListener('paste', function (event) {
    try {
        ...
        Alerta.TratamentoErroComLinha(
            'agendamento_viagem.js',
            'addEventListener:paste_callback',
            error,
        );
    }
});
...
function limparCamposModalViagens() {
    try {
        ...
        if (lstFinalidade) {
            ...
            console.warn(
                'Syncfusion instance #lstFinalidade não existe ou ainda não foi inicializada.',
            );
        }
        ...
        $(
            '#divPeriodo, #divTxtPeriodo, #divDias, #divDiaMes, #divFinalRecorrencia, #divFinalFalsoRecorrencia, #calendarContainer, #listboxContainer, #listboxContainerHTML, #ReportContainer',
        ).hide();
