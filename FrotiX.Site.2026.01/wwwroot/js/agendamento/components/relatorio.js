 ...
 * ╠══════════════════════════════════════════════════════════════════════════╣
 * ║ ❓ POR QUE EXISTO?                                                       ║
 * ║    Gerenciamento do Telerik Report Viewer para Ficha de Vistoria.        ║
 * ║    • mostrarLoadingRelatorio() - Overlay padrão FrotiX                   ║
 * ║    • esconderLoadingRelatorio() - Remove overlay após carregamento       ║
 * ║    • Integração com Telerik Report Viewer                                ║
 * ║    • Recuperação de viagem por ID                                        ║
 * ║                                                                          ║
 * ║ 🔗 RELEVÂNCIA: Alta (Agenda - Relatório)                                 ║
 * ╚══════════════════════════════════════════════════════════════════════════╝
 */
...
/**
 * 🎯 MÓDULO DE RELATÓRIO
 * @description Gerencia a exibição de relatórios Telerik no modal de visualização
 */
(function () {
    'use strict';
    ...
    window.esconderLoadingRelatorio = function () {
        ...
        }, 2000); // Aumentado de 1s para 2s para dar tempo da Ficha carregar
    };
    ...
    window.carregarRelatorioViagem = async function (viagemId) {
        ...
        `);
            ...
            // ✅ REMOVER OVERLAY IMEDIATAMENTE EM CASO DE ERRO
            window.esconderLoadingRelatorio();
                ...
                try {
                    ...
                    if ($viewer.length > 0) {
                        ...
                        if (instance) {
                            ...
                            try {
                                // Verificar se o viewer está em estado válido antes de destruir
                                // O erro 'collapsible' ocorre quando Kendo não inicializou completamente
                                const isInitialized =
                                ...
                                // Silenciar erros de Kendo durante destruição incompleta
                                if (
                                    e.message &&
