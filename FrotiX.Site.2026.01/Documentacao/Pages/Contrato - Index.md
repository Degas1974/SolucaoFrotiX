...
function loadList() {
    dataTable = $("#tblContrato").DataTable({
        ordering: false, // Ordenação desabilitada (dados já vêm ordenados do backend)
...
## [15/01/2026] - Remoção de Ordenação Visual no Header

**Descrição**:
Removida a funcionalidade de ordenação do DataTable (`ordering: false`) para eliminar as setas do cabeçalho, mantendo a ordenação original trazida pelo backend.
