...
## [23/01/2026] - Correção de escape @ em JSDoc

**Descrição**:
Escapado caractere `@` para `@@` em todos os comentários JSDoc dentro de blocos `<script>`.
Em arquivos Razor, `@` é interpretado como sintaxe Razor, causando erros de compilação CS0103.

**Arquivos Afetados**:
- `Pages/Abastecimento/RegistraCupons.cshtml`

**Tags corrigidas**: `@description`, `@type`, `@param`, `@returns`, `@this`

**Impacto**: Correção de erros de compilação
