...
-- Script date 09/02/2026 08:25:23
...
--
PRINT (N'Create table [dbo].[SchemaChangeLog]')
GO
CREATE TABLE dbo.SchemaChangeLog (
  Id bigint IDENTITY,
...
GO
EXEC sys.sp_addextendedproperty N'MS_Description', N'Tabela de logs de erros do sistema FrotiX (servidor e cliente). Armazena todos os erros, warnings e informaÃ§Ãµes para anÃ¡lise e dashboard.', 'SCHEMA', N'dbo', 'TABLE', N'LogErros'
GO
...
CREATE INDEX IX_DocGen_FileChangeHistory_FilePath_DetectedAt
  ON DocGenerator.FileChangeHistory (FilePath, DetectedAt)
  ON [PRIMARY]
GO
...
--
-- Create or alter function [dbo].[fn_PascalCasePtBr]
--
GO
PRINT (N'Create or alter function [dbo].[fn_PascalCasePtBr]')
GO
CREATE OR ALTER FUNCTION dbo.fn_PascalCasePtBr (@s NVARCHAR(4000))
RETURNS NVARCHAR(4000)
AS
BEGIN
    IF @s IS NULL RETURN NULL;

    -- Normaliza espaços (inclui TAB e NBSP)
    SET @s = REPLACE(REPLACE(@s, CHAR(9), N' '), NCHAR(160), N' ');
    SET @s = LTRIM(RTRIM(@s));
    WHILE CHARINDEX(N'  ', @s) > 0
        SET @s = REPLACE(@s, N'  ', N' ');

    -- Base em minúsculas
    DECLARE @base NVARCHAR(4000) = LOWER(@s);

    -- Capitaliza início de palavra (espaço, hífen, apóstrofo)
    DECLARE @i INT = 1, @len INT = LEN(@base);
    DECLARE @out NVARCHAR(4000) = N'';
    DECLARE @ch NCHAR(1), @prev NCHAR(1);

    WHILE @i <= @len
    BEGIN
        SET @ch = SUBSTRING(@base, @i, 1);
        SET @prev = CASE WHEN @i = 1 THEN N' ' ELSE SUBSTRING(@base, @i - 1, 1) END;

        IF @i = 1 OR @prev IN (N' ', N'-', N'''')
            SET @out += UPPER(@ch);
        ELSE
            SET @out += @ch;
    ...
    -- Rebaixa partículas (exceto se for a 1ª palavra)
    DECLARE @result NVARCHAR(4000) = N'';
    DECLARE @pos INT = 1, @next INT, @word NVARCHAR(4000), @wLower NVARCHAR(4000);
    DECLARE @wordIndex INT = 0;

    WHILE @pos <= LEN(@out)
    BEGIN
        SET @next = CHARINDEX(N' ', @out, @pos);
        IF @next = 0 SET @next = LEN(@out) + 1;

        SET @word = SUBSTRING(@out, @pos, @next - @pos);
        SET @wLower = LOWER(@word);
        SET @wordIndex += 1;

        IF @wordIndex > 1 AND @wLower IN (N'de', N'da', N'do', N'das', N'dos', N'e', N'em', N'no', N'na', N'nos', N'nas')
            SET @word = @wLower;

        SET @result = CASE WHEN @result = N'' THEN @word ELSE @result + N' ' + @word END;
