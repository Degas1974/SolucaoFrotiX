/* ======================================================================================
   SCRIPT INTEGRADO COM ROLLBACK - ATUALIZAÃ‡ÃƒO COMPLETA FROTIX  
   ======================================================================================
   
   âš ï¸âš ï¸âš ï¸  BACKUP OBRIGATÃ“RIO ANTES DE EXECUTAR  âš ï¸âš ï¸âš ï¸
   
   DATA GERAÃ‡ÃƒO: 09/02/2026 09:03:29
   
   ESTE SCRIPT CONTÃ‰M:
   - FASE 1: MigraÃ§Ã£o de 5 tabelas existentes (ALTER TABLE) âœ… COM ROLLBACK
   - FASE 2: CriaÃ§Ã£o de 31 tabelas + 67 views + 20 procedures âš ï¸  SEM ROLLBACK (DDL)
...
-- ======================================================================================
-- FASE 1: MIGRAÃ‡ÃƒO DE TABELAS EXISTENTES (COM TRANSAÃ‡Ã•ES E ROLLBACK)
-- ======================================================================================
...
-- Verificar estado da coluna HasChild
...
ELSE
BEGIN
    PRINT '  ℹ️  Coluna HasChild já existe.';
END
GO

-- Atualizar valores de HasChild baseado na existência de filhos (SQL DINÂMICO)
...
BEGIN
    ...
    BEGIN TRY
        ...
        PRINT '  ✓ Recursos marcados com HasChild = 1';
        PRINT '✅ Coluna HasChild criada e populada com sucesso!';
        
    END TRY
    BEGIN CATCH
        PRINT '❌ ERRO ao atualizar HasChild: ' + ERROR_MESSAGE();
        THROW;
    END CATCH
END
ELSE
...
BEGIN TRY
    ...
    IF EXISTS (SELECT * FROM sys.indexes WHERE name = 'IX_Recurso_NomeMenu' AND object_id = OBJECT_ID('dbo.Recurso'))
    BEGIN
        PRINT '  → Removendo índice IX_Recurso_NomeMenu...';
        DROP INDEX [IX_Recurso_NomeMenu] ON [dbo].[Recurso];
    END
...
BEGIN TRY
    ...
    -- NÃO alterar tipo - manter nvarchar(450) compatível com AspNetUsers.Id
    PRINT '  ℹ️  Pulando alteração de tipo (AspNetUsers.Id é nvarchar(450)).';
    
    -- Recriar FKs removidas (sem alterar tipo)
...
-- ======================================================================================
-- Prosseguir sem verificação de log (tabela não implementada)
GO

-- ======================================================================================
-- FASE 2: CRIAÃ‡ÃƒO DE NOVOS OBJETOS
-- ======================================================================================
...
-- ======================================================================================
-- SCRIPT DE ROLLBACK MANUAL (USAR APENAS SE NECESSÃRIO)
-- ======================================================================================
/*
...
-- Remover objetos da Fase 2 (se necessÃ¡rio)
