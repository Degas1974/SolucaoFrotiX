...
**Mudanca:** GRANDE | **+702** linhas | **-902** linhas

---

```diff
--- JANEIRO: wwwroot/js/cadastros/ListaManutencao.js
+++ ATUAL: wwwroot/js/cadastros/ListaManutencao.js
@@ -1,188 +1,170 @@
-var URLapi = '';
-var IDapi = '';
-
...
-                    Alerta.TratamentoErroComLinha(
-                        'ListaManutencao.js',
-                        'ajax.error@carregaManutencaoInicial',
-                        thrown,
-                    );
-                },
...
-                            const isClosed =
-                                status === 'Fechada' || status === 'Cancelada';
-
-                            const tooltipEditar = isClosed
-                                ? 'Visualizar OS'
-                                : 'Editar OS';
-                            const tooltipBaixar = isClosed
-                                ? 'Desabilitado'
-                                : 'Baixar OS';
...
-                                      : 'Cancelar OS';
-
-                            const disabledAttr = isClosed
-                                ? 'aria-disabled="true"'
-                                : '';
-                            const iconEditar = isClosed
-                                ? 'fa-duotone fa-eye'
-                                : 'fa-duotone fa-pen-to-square';
-                            const iconBaixar = isClosed
-                                ? 'fa-duotone fa-lock'
...
-                Alerta.Erro(
-                    'Informação Ausente',
-                    'Informe Mês e Ano (ou use Período com as duas datas).',
-                    'Ok',
-                );
+        if (temIni && temFim)
+        {
...
-
-        var ManutencaoId =
-            $('#txtId').val() || $('#btnFecharManutencao').data('id');
-
-        if (!ManutencaoId) {
...
+                    { targets: 5, visible: true, className: "text-center", width: "12%" },
+                    { targets: 6, visible: false },
+                    { targets: 7, visible: false },
+                    { targets: 8, visible: false },
+                    { targets: 9, visible: false },
...
-                    AppToast.show(
-                        'Amarelo',
-                        'Item removido. Ficará como PENDENTE após a baixa.',
-                        3000,
-                    );
-
-                    console.log(
-                        '[ListaManutencao.js] Itens removidos para Pendente:',
-                        itensRemovidosParaPendente,
-                    );
-                } catch (error) {
-                    Alerta.TratamentoErroComLinha(
-                        'ListaManutencao.js',
-                        'btn-remover-item-baixa.confirm',
-                        error,
-                    );
...
-            msgConfirm +=
-                '\n\n⚠️ ' +
-                itensRemovidosParaPendente.length +
-                ' item(ns) removido(s) ficará(ão) como PENDENTE.';
-        }
-
-        Alerta.Confirmar('Confirma Baixa?', msgConfirm, 'Sim', 'Não').then(
-            function (confirmado) {
-                if (confirmado) {
+        var msgConfirm = "Deseja baixar esta Ordem de Serviço?";
+        if (itensRemovidosParaPendente.length > 0)
...
-                            itensRemovidosJson: JSON.stringify(
-                                itensRemovidosParaPendente,
-                            ),
...
-                                    var msg =
-                                        'Ordem de Serviço baixada com sucesso!';
-                                    if (itensRemovidosParaPendente.length > 0) {
-                                        msg +=
-                                            ' (' +
-                                            itensRemovidosParaPendente.length +
...
-
-        var numeroOS = dataRow.numOS || '';
-        var $osNumero = $('#modalManutencaoOsNumero');
-        if ($osNumero.length) {
-            if (numeroOS) {
-                $osNumero.text('(OS: ' + numeroOS + ')').removeClass('d-none');
-            } else {
-                $osNumero.text('').addClass('d-none');
-            }
-        }
-
