...
├── ControlesKendo.md                ← GUIA KENDO/TELERIK (CONSULTAR SEMPRE)
...
## 0.1 CONTROLES KENDO/TELERIK - CONSULTA OBRIGATORIA

### Regra: Consultar ControlesKendo.md Antes de Usar Controles Telerik

**Contexto:** O projeto FrotiX utiliza exclusivamente controles Telerik Kendo UI (versao 2025.4.1321) para inputs de data, hora, dropdowns, grids, editors e uploads. O arquivo `ControlesKendo.md` (raiz do workspace) contem a documentacao completa de TODOS os controles com sintaxe TagHelper, HtmlHelper, acesso JavaScript e exemplos reais do FrotiX.

**Regra:** SEMPRE consultar `ControlesKendo.md` ANTES de:

1. **Criar** qualquer controle Kendo em paginas CSHTML
2. **Modificar** controles Kendo existentes
3. **Acessar** widgets Kendo via JavaScript (`.data("kendo...")`)
4. **Criar** novos helpers JavaScript para widgets Kendo
5. **Escolher** entre DropDownList vs ComboBox vs MultiSelect

**O arquivo contem:**

- Sintaxe TagHelper e HtmlHelper para cada controle
- Atributos disponiveis com tipos e exemplos
- API JavaScript completa (get/set/enable/disable)
- Exemplos reais do FrotiX.Site.OLD
- Helpers JS existentes (kendo-datetime.js, kendo-editor-upsert.js)
- Erros comuns e solucoes
- Referencia cruzada com `Kendo.Mvc.Examples/`

**Data de Adicao:** 09/02/2026
...
### 3.3 Telerik/Kendo – Carregamento e Localização (OBRIGATÓRIO)

**REGRA INVIOLÁVEL:** Kendo UI é carregado **UMA ÚNICA VEZ**, globalmente, via `_ScriptsBasePlugins.cshtml`. Cultura pt-BR já configurada globalmente. **NUNCA** carregar Kendo CDN em páginas individuais.

**Carregamento global (já configurado em `_ScriptsBasePlugins.cshtml`):**

```html
<!-- _ScriptsBasePlugins.cshtml (NÃO DUPLICAR em páginas) -->
...
**CSS global (já configurado em `_Head.cshtml`):**
...
**Proibições:**
- **NUNCA** adicionar `<script src="https://kendo.cdn.telerik.com/...">` em páginas individuais
- **NUNCA** adicionar `<link href="https://kendo.cdn.telerik.com/...">` em páginas individuais
- **NUNCA** duplicar `kendo.culture("pt-BR")` em páginas (já é global)
- **NUNCA** carregar Kendo 2022.x (conflita com tema bootstrap-main do 2025)

**Licença:** O arquivo `~/kendo-lic.js` (copiado de `Kendo.Mvc.Examples`) ativa a licença client-side via `KendoLicensing.setScriptKey()`. Sem ele, componentes mostram erro de licenciamento.

### 3.3.1 Telerik/Kendo – Inicialização de Controles (OBRIGATÓRIO)

**REGRA INVIOLÁVEL:** Controles Kendo DEVEM ser inicializados via **jQuery** dentro de `@section ScriptsBlock {}`, que executa APÓS `_ScriptsBasePlugins.cshtml` carregar `kendo.all.min.js`.

**Motivo:** O Layout renderiza nesta ordem:
1. `@RenderBody()` → HTML da página (controles são `<input>` simples)
2. `_ScriptsBasePlugins.cshtml` → carrega `kendo.all.min.js`
3. `@RenderSection("ScriptsBlock")` → inicializa controles (kendo JÁ existe)

**NUNCA usar TagHelpers Kendo** (`<kendo-dropdownlist>`, `<kendo-combobox>`, `<kendo-datepicker>`). Eles geram scripts inline no `@RenderBody()`, ANTES do kendo.all.min.js carregar, causando `ReferenceError: kendo is not defined`.

**✅ CORRETO – HTML simples + jQuery init no ScriptsBlock:**

```html
<!-- No body da página: input simples como placeholder -->
<input id="meuDropDown" name="Model.Campo" style="width: 100%;" />
```

```javascript
// No @section ScriptsBlock { } - roda APÓS kendo.all.min.js
@section ScriptsBlock {
    <script>
        (function () {
            try {
                var dados = @Html.Raw(Json.Serialize(ViewData["meusDados"]));

                $("#meuDropDown").kendoDropDownList({
                    dataTextField: "descricao",   // camelCase (JSON)
                    dataValueField: "id",          // camelCase (JSON)
                    optionLabel: "Selecione...",
                    dataSource: dados,
                    height: 200,
                    value: "@(Model.ValorAtual ?? "")"
                });
            } catch (error) {
                Alerta.TratamentoErroComLinha("Pagina.cshtml", "kendoInit", error);
            }
        })();
    </script>
}
...
**Acesso ao controle via JavaScript:**

```javascript
// Obter instância do widget
var ddl = $("#meuDropDown").data("kendoDropDownList");
if (ddl) {
    ddl.value("novoValor");     // setar valor
    ddl.enable(false);          // desabilitar
    var val = ddl.value();      // obter valor
}
```

**JSON Serialization:** `@Html.Raw(Json.Serialize(...))` usa **camelCase** (ASP.NET Core default). Propriedades C# `Descricao` viram `descricao` no JSON. Templates JS devem usar `data.descricao`, não `data.Descricao`.

**Checklist de Auditoria Kendo:**
1. Nenhuma página carrega Kendo CDN (`kendo.cdn.telerik.com` ou `cdn.kendostatic.com`)?
2. Controles inicializados via jQuery no `@section ScriptsBlock`, não TagHelpers?
3. Propriedades no JS usam camelCase (compatível com JSON serialization)?
4. Nenhuma duplicação de cultura/messages em páginas individuais?
5. `kendo-lic.js` referenciado em `_ScriptsBasePlugins.cshtml` após `kendo.all.min.js`?
...
### REGRA DE PRIORIDADE: Arquivos de Configuração de IA

**REGRA INVIOLÁVEL:** Quando QUALQUER arquivo de configuração de IA for atualizado (`MEMORY.md`, `CLAUDE.md`, `.cursorrules`, `.continuerules`, `.aicontext`, `copilot-instructions.md`), as informações relevantes DEVEM ser **primariamente gravadas neste arquivo** (`RegrasDesenvolvimentoFrotiX.md`).

**Hierarquia:**
1. **`RegrasDesenvolvimentoFrotiX.md`** → FONTE OFICIAL (este arquivo). Todas as regras vão aqui PRIMEIRO.
2. **Outros arquivos de IA** → Apenas redirecionam para este. Não devem conter regras exclusivas.
3. **`MEMORY.md` (Claude Code)** → Notas técnicas internas do Claude. Regras do projeto NÃO vão lá.

**Motivo:** Evitar fragmentação de regras entre múltiplos arquivos. Um único arquivo é a verdade.
