...
| `orderColumn` | int | 1 | Indice da coluna para ordenacao (0-7) |
| `orderDir` | string | "desc" | Direcao da ordenacao ("asc" ou "desc") |
...
2. **PASSO 2**: Buscar eventos da pagina atual com ordenacao dinamica
   - Mapeia `orderColumn` (0-7) para campo do banco
   - Aplica ordenacao ASC ou DESC baseado em `orderDir`
   - Colunas ordenáveis: Nome, DataInicial, DataFinal, QtdParticipantes, Setor, Status
   - Coluna Custo ordenada em memoria (PASSO 4)
   - Include de SetorSolicitante e Requisitante
...
## [25/01/2026] - Adicao de ordenacao dinamica server-side

**Descricao**:

- Adicionados parametros `orderColumn` (int) e `orderDir` (string) ao endpoint
- Implementado switch para mapear indices de coluna (0-7) aos campos do banco
- Suporte a ordenacao por: Nome, DataInicial, DataFinal, QtdParticipantes, Setor, Status
- Ordenacao da coluna Custo (indice 5) realizada em memoria apos carregar dados
- Ordenacao padrao: DataInicial descendente (coluna 1)

**Mapeamento de Colunas**:

| Indice | Campo Database | Ordenavel |
|--------|----------------|-----------|
| 0 | e.Nome | Sim |
| 1 | e.DataInicial | Sim (padrao) |
| 2 | e.DataFinal | Sim |
| 3 | e.QtdParticipantes | Sim |
| 4 | e.SetorSolicitante.Nome | Sim |
| 5 | custoViagem | Sim (memoria) |
| 6 | e.Status | Sim |
| 7 | N/A (acoes) | Nao |
...
**Impacto**: DataTables agora processa ordenacao corretamente ao clicar nas setas das colunas
