 ...
 * --------------------------------------------------------------------------------------
 * 🎯 OBJETIVO     : PageModel para a página de lockout, permitindo desbloqueio
 *                   mediante confirmação de senha.
 *
 * 📥 ENTRADAS     : Input.Password (senha), returnUrl (opcional).
 *
 * 📤 SAÍDAS       : IActionResult (redirect ou Page), mensagens em ViewData/TempData.
 *
 * 🔗 CHAMADA POR  : Motor Razor (GET/POST /Account/Lockout).
 *
 * 🔄 CHAMA        : SignInManager.PasswordSignInAsync(), SignInManager.SignOutAsync().
 *
 * 📦 DEPENDÊNCIAS : ASP.NET Core Identity, ILogger, Razor Pages.
 *
 * 📝 OBSERVAÇÕES  : OnGetAsync executa SignOut (comportamento questionável; revisar).
 **************************************************************************************** */
