...
namespace FrotiXApi.Data
{
    public partial class FrotiXDbContext : DbContext
    {
        ...
        {
            // Configuração específica para Viagem
            modelBuilder.Entity<Viagem>(entity =>
            {
                // Chave primária
                entity.HasKey(e => e.ViagemId);

                // ⚡ IMPORTANTE: Desabilita OUTPUT clause para compatibilidade com triggers
                entity.ToTable(tb => tb.UseSqlOutputClause(false));

                // Configurar campos nullable explicitamente para evitar erro SqlNullValueException
                entity.Property(e => e.SetorSolicitanteId).IsRequired(false);
                entity.Property(e => e.EventoId).IsRequired(false);
                entity.Property(e => e.MotoristaId).IsRequired(false);
                entity.Property(e => e.VeiculoId).IsRequired(false);
                entity.Property(e => e.RequisitanteId).IsRequired(false);
                entity.Property(e => e.ItemManutencaoId).IsRequired(false);
                entity.Property(e => e.RecorrenciaViagemId).IsRequired(false);
                ...
                entity
                    .HasOne(v => v.SetorSolicitante)
                    .WithMany()
                    .HasForeignKey(v => v.SetorSolicitanteId)
                    .OnDelete(DeleteBehavior.SetNull);

                entity
                    .HasOne(v => v.Requisitante)
                    .WithMany()
                    .HasForeignKey(v => v.RequisitanteId)
                    .OnDelete(DeleteBehavior.SetNull);

                entity
                    .HasOne(v => v.ItensManutencao)
                    .WithMany()
                    .HasForeignKey(v => v.ItemManutencaoId)
                    .OnDelete(DeleteBehavior.SetNull);
            });

            // Configurações para chaves compostas
            modelBuilder
