...
**Descrição**: Substituída classe Bootstrap outline genérica `btn-outline-secondary` por `btn-vinho` (padrão FrotiX sólido) em botão de limpar filtros.

**Problema Identificado**:
- Uso de classe Bootstrap outline genérica `btn-outline-secondary` em botão de limpar filtros
...
- Botão "Limpar filtro" (linha 355): mudado de `btn-outline-secondary` para `btn-vinho`
...
- Consistência com Fases 1, 2 e 3 do projeto de padronização

**Arquivos Afetados**:
- Pages/Administracao/LogErros.cshtml (linha 355)
